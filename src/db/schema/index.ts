...
export const playerState = pgEnum("player_state", [
  "available",
  "playing",
  "waiting",
  "resting",
...
export const venueParkingStatus = pgEnum("venue_parking_status", [
  "available",
  "unavailable",
]);
...
export const venueChangeRequestType = pgEnum("venue_change_request_type", [
  "create",
  "update",
]);
...
export const users = pgTable(
  ...
  (table) => [
    index("users_created_id_idx").on(table.createdAt.desc(), table.id.desc()),
  ]
);
...
export const venues = pgTable(
  "venues",
  {
    ...
    reservationPolicy: venueReservationPolicy("reservation_policy")
      .notNull()
      .default("unknown"),
    operationalStatus: venueOperationalStatus("operational_status")
      .notNull()
      .default("unknown"),
    ...
    listingStatus: venueListingStatus("listing_status")
      .notNull()
      .default("verified"),
  ...
  (table) => [
    uniqueIndex("venues_source_external_id_idx").on(
      table.source,
      table.sourceExternalId
    ),
    index("venues_updated_id_idx").on(table.updatedAt.desc(), table.id.desc()),
    index("venues_listing_environment_idx").on(
      table.listingStatus,
      table.environment
    ),
    index("venues_listing_parking_idx").on(
      table.listingStatus,
      table.parkingStatus
    ),
    index("venues_listing_price_idx").on(
      table.listingStatus,
      table.priceStatus,
      table.priceAmountCents
    ),
    check(
      "venues_price_amount_nonnegative",
      sql`${table.priceAmountCents} is null or ${table.priceAmountCents} >= 0`
    ),
    check(
...
export const venueChangeRequests = pgTable(
  ...
  (table) => [
    index("venue_change_requests_status_created_idx").on(
      table.status,
      table.createdAt.desc(),
      table.id.desc()
    ),
    index("venue_change_requests_venue_created_idx").on(
      table.venueId,
      table.createdAt.desc()
    ),
    index("venue_change_requests_submitter_created_idx").on(
      table.submittedById,
      table.createdAt.desc()
    ),
...
export const venueOperatingPeriods = pgTable(
  ...
  (table) => [
    ...
    index("venue_operating_periods_venue_day_idx").on(
      table.venueId,
      table.dayOfWeek
    ),
    check(
      "venue_operating_periods_day_valid",
      sql`${table.dayOfWeek} between 1 and 7`
    ),
    check(
      "venue_operating_periods_sequence_nonnegative",
...
export const groupMembers = pgTable(
  ...
  (table) => [
    ...
    index("group_members_user_joined_idx").on(
      table.userId,
      table.joinedAt,
      table.groupId
    ),
...
export const sessions = pgTable(
  "sessions",
  {
    ...
    rotationConfig: jsonb("rotation_config")
      .$type<Record<string, unknown>>()
      .notNull()
      .default({}),
  ...
  (table) => [
    ...
    index("sessions_group_starts_id_idx").on(
      table.groupId,
      table.startsAt.desc(),
      table.id.desc()
    ),
...
export const sessionPlayers = pgTable(
  ...
  (table) => [
    ...
    index("session_players_user_rsvp_idx").on(
      table.userId,
      table.rsvp,
      table.sessionId
    ),
...
export const courts = pgTable(
  ...
  (table) => [
    unique("session_court_position_unique").on(table.sessionId, table.position),
  ]
);
...
export const matches = pgTable(
  ...
  (table) => [
    check(
      "match_scores_nonnegative",
      sql`${table.teamAScore} >= 0 and ${table.teamBScore} >= 0`
    ),
  ]
);
...
export const matchPlayers = pgTable(
  ...
  (table) => [
    ...
    index("match_players_session_player_idx").on(
      table.sessionPlayerId,
      table.matchId
    ),
  ]
);
...
export const matchScores = pgTable(
  ...
  (table) => [
    unique("match_score_sequence_unique").on(table.matchId, table.sequence),
  ]
);
...
export const sessionPairMembers = pgTable(
  ...
  (table) => [
    ...
    unique("session_pair_member_position_unique").on(
      table.pairId,
      table.position
    ),
    unique("session_pair_player_unique").on(table.sessionPlayerId),
    check(
      "session_pair_member_position_valid",
...
export const messages = pgTable(
  "messages",
  {
    ...
    sessionPlayerId: uuid("session_player_id").references(
      () => sessionPlayers.id,
      { onDelete: "restrict" }
    ),
  ...
  (table) => [
    index("messages_session_created_id_idx").on(
      table.sessionId,
      table.createdAt.desc(),
      table.id.desc()
    ),
...
export const messageReactions = pgTable(
  ...
  (table) => [
    unique("message_player_reaction_unique").on(
      table.messageId,
      table.sessionPlayerId,
      table.reaction
    ),
...
export const reactions = pgTable(
  ...
  (table) => [
    primaryKey({ columns: [table.memoryId, table.userId, table.reaction] }),
  ]
);
...
export const notificationPreferences = pgTable("notification_preferences", {
  ...
  emailCategories: jsonb("email_categories")
...
export const pushSubscriptions = pgTable(
  ...
  (table) => [
    index("push_subscriptions_user_idx").on(table.userId, table.createdAt),
  ]
);
...
export const notificationDeliveries = pgTable(
  ...
  (table) => [
    unique("notification_delivery_destination_unique").on(
      table.notificationId,
      table.channel,
      table.destinationKey
    ),
    index("notification_delivery_pending_idx").on(
      table.status,
      table.nextAttemptAt
    ),
  ]
...
export const productEvents = pgTable(
  ...
  (table) => [
    ...
    index("product_events_session_created_idx").on(
      table.sessionId,
      table.createdAt
    ),
  ]
...
export const feedbackSubmissions = pgTable(
  ...
  (table) => [
    ...
    index("feedback_created_id_idx").on(
      table.createdAt.desc(),
      table.id.desc()
    ),
  ]
...
export const signupSettings = pgTable(
  ...
  (table) => [
    ...
    check(
      "signup_account_cap_valid",
      sql`${table.accountCap} between 1 and 50000`
    ),
  ]
);
...
export const adminAuditLogs = pgTable(
  ...
  (table) => [
    ...
    index("admin_audit_created_id_idx").on(
      table.createdAt.desc(),
      table.id.desc()
    ),
