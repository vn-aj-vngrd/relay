...
import { openGamesFilterSchema, parseOpenGameCursor } from "@/features/sessions/open-games";
...
export async function GET(request: NextRequest) {
  ...
  const limit = await checkRateLimit({ scope: "open-games", limit: 120, windowSeconds: 60 }, `user:${user.id}`);
  if (!limit.allowed)
    return Response.json(
  ...
  try {
    return Response.json(await discoverOpenGames(user.id, parsedFilters.data, cursor), {
      headers: { ...rateLimitHeaders(limit), "Cache-Control": "private, no-store" },
    });
  } catch (error) {
    console.error("Open games loading failed", error instanceof Error ? error.message : "Unknown error");
    return Response.json(
