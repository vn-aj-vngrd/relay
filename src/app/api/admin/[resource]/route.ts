...
import { getAdminAuditLog, getAdminSessions, getAdminUsers, getAdminVenues } from "@/features/admin/queries";
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ resource: string }> }) {
  ...
  const limit = await checkRateLimit({ scope: "admin-pagination", limit: 180, windowSeconds: 60 }, `user:${admin.id}`);
  if (!limit.allowed)
    return Response.json(
  ...
  try {
    ...
    console.error("Admin pagination failed", resource.data, error instanceof Error ? error.message : "Unknown error");
    return Response.json(
