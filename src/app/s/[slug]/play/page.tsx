...
export default async function PublicPlayPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const sessionViewer = data.session.status === "completed" ? null : await getSessionViewer(data.session.id, slug);
  const viewer: SessionPlayViewer = {
    ...
    canScoreAll: Boolean(
      sessionViewer?.user && (data.session.hostId === sessionViewer.user.id || sessionViewer.player.role === "cohost"),
    ),
      ...
      <div className="public-session-content mx-auto w-full max-w-6xl bg-surface px-4 pb-8 pt-4 sm:px-6 sm:py-8">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div className="min-w-0">
            <h1 className="public-tab-title app-title">{completed ? "Recap" : "Play"}</h1>
        ...
        <div className={completed ? "sm:mt-7" : undefined}>
          <SessionPlay data={data} viewer={viewer} storyHref={`/s/${slug}/story`} />
        </div>
      </div>
    </main>
  );
