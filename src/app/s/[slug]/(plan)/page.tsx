...
import { CaretRight, Heart, UploadSimple, Users } from "@phosphor-icons/react/dist/ssr";
...
export default async function PublicSessionPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main id="main-content" className="public-session-page min-h-screen bg-canvas" style={sessionAccentStyle(session.accentColor)}>
    <div className={`mx-auto grid gap-6 px-0 pb-12 sm:px-6 sm:pt-8 ${session.status === "completed" ? "max-w-3xl" : "max-w-[1040px] lg:grid-cols-[1fr_350px]"}`}><article className="public-session-panel min-w-0 bg-surface sm:rounded-xl sm:border sm:border-line"><SessionHero session={session} hostLabel={`Hosted by ${hostProfile?.name ?? "the host"}`} />
