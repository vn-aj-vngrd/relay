...
function JoinPanelSkeleton({ mobile = false }: { mobile?: boolean }) {
  return <div className={mobile ? "public-session-section border-b border-line lg:hidden" : "public-session-panel bg-surface p-5 sm:rounded-xl sm:border sm:border-line"}>
    ...
    <div className="mt-3 grid grid-cols-3 gap-2"><Skeleton className="h-9" /><Skeleton className="h-9" /><Skeleton className="h-9" /></div><Skeleton className="mt-3 h-9 w-full" /><Skeleton className="mx-auto mt-5 h-4 w-24" />
  </div>;
}

function RosterSkeleton({ mobile = false }: { mobile?: boolean }) {
  return <section className={mobile ? "public-session-section border-b border-line lg:hidden" : ""}><div className="mb-3 flex items-end justify-between gap-3"><div className="space-y-2"><Skeleton className="h-5 w-32" /><Skeleton className="h-3.5 w-40" /></div><div className="flex -space-x-2"><Skeleton className="h-8 w-8 rounded-full" /><Skeleton className="h-8 w-8 rounded-full" /><Skeleton className="h-8 w-8 rounded-full" /></div></div><div className="divide-y divide-line border-y border-line">{Array.from({ length: 3 }, (_, index) => <div key={index} className="flex min-h-14 items-center gap-3 py-2"><Skeleton className="h-8 w-8 rounded-full" /><Skeleton className="h-3.5 flex-1" /><Skeleton className="h-3 w-10" /></div>)}</div><Skeleton className="mx-auto mt-3 h-9 w-32" /></section>;
}
