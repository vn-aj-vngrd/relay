...
export default async function HomePage() {
  ...
  return <div className="space-y-12 sm:space-y-16">
    ...
    {next ? <section aria-labelledby="next-game-heading">
      ...
      <div className="mt-4 grid divide-y divide-line border-y border-line sm:grid-cols-3 sm:divide-x sm:divide-y-0"><Link href={`/games/${next.session.id}/players`} prefetch={false} className="pressable flex min-h-14 items-center gap-3 py-3 hover:bg-surface-strong sm:px-3"><UsersThree size={18} className="text-muted" /><span><span className="block text-sm font-medium">Players</span><span className="mt-0.5 block text-xs text-muted">{next.playerCount} of {next.session.capacity} going</span></span></Link><Link href={`/games/${next.session.id}/payments`} prefetch={false} className="pressable flex min-h-14 items-center gap-3 py-3 hover:bg-surface-strong sm:px-3"><CurrencyCircleDollar size={18} className="text-muted" /><span><span className="block text-sm font-medium">Payments</span><span className="mt-0.5 block text-xs text-muted">View the split</span></span></Link><Link href={`/games/${next.session.id}/play`} prefetch={false} className="pressable flex min-h-14 items-center gap-3 py-3 hover:bg-surface-strong sm:px-3"><SquaresFour size={18} className="text-muted" /><span><span className="block text-sm font-medium">Play</span><span className="mt-0.5 block text-xs text-muted">Courts, queue, and scores</span></span></Link></div>
