...
import { ArrowRight, CalendarBlank, CaretRight, CurrencyCircleDollar, MapPin, SquaresFour, Users, UsersThree } from "@phosphor-icons/react/dist/ssr";
...
export default async function HomePage() {
  ...
  return <div className="space-y-12 sm:space-y-16">
    ...
    {next ? <section aria-labelledby="next-game-heading" style={sessionAccentStyle(next.session.accentColor)}>
      ...
      <article className="overflow-hidden rounded-xl bg-[var(--session-cover)] text-white ring-1 ring-black/5">
        <div className="grid md:grid-cols-[1fr_260px]">
          ...
          <Link href={nextHref} prefetch={false} className="pressable group flex min-h-20 items-center justify-between bg-primary px-5 py-4 font-semibold hover:bg-primary-hover md:flex-col md:items-start md:justify-end md:p-6"><span><span className="block text-sm text-white/75">Everything in one place</span><span className="mt-1 block text-lg">{next.player.rsvp === "invited" ? "Review invite" : "View game"}</span></span><ArrowRight className="transition-transform group-hover:translate-x-1" /></Link>
        </div>
      </article>
      <div className="mt-4 grid divide-y divide-line border-y border-line sm:grid-cols-3 sm:divide-x sm:divide-y-0"><Link href={`${nextHref}/players`} prefetch={false} className="pressable flex min-h-14 items-center gap-3 py-3 hover:bg-surface-strong sm:px-3"><UsersThree size={18} className="text-muted" /><span><span className="block text-sm font-medium">Players</span><span className="mt-0.5 block text-xs text-muted">{next.playerCount} of {next.session.capacity} going</span></span></Link><Link href={`${nextHref}/payments`} prefetch={false} className="pressable flex min-h-14 items-center gap-3 py-3 hover:bg-surface-strong sm:px-3"><CurrencyCircleDollar size={18} className="text-muted" /><span><span className="block text-sm font-medium">Payments</span><span className="mt-0.5 block text-xs text-muted">View the split</span></span></Link><Link href={`${nextHref}/play`} prefetch={false} className="pressable flex min-h-14 items-center gap-3 py-3 hover:bg-surface-strong sm:px-3"><SquaresFour size={18} className="text-muted" /><span><span className="block text-sm font-medium">Play</span><span className="mt-0.5 block text-xs text-muted">Courts, queue, and scores</span></span></Link></div>
      ...
      <div className="py-8 lg:border-r lg:border-line lg:py-10 lg:pr-12">
        ...
        <div className="mt-7 flex flex-wrap items-center gap-4"><ButtonLink href="/games/new">Create your first game <ArrowRight size={17} /></ButtonLink><Link href="/home?tour=1" className="inline-flex min-h-11 items-center text-sm font-semibold text-muted hover:text-ink">Replay the short tour</Link></div>
      </div>
    ...
    {data.upcoming.length > 1 ? <section aria-labelledby="upcoming-heading"><div className="mb-3 flex items-center justify-between"><h2 id="upcoming-heading" className="text-lg font-bold">Upcoming games</h2><Link href="/games" className="inline-flex min-h-11 items-center text-sm font-semibold text-primary">View all</Link></div><div className="divide-y divide-line border-y border-line">{data.upcoming.slice(1, 4).map(({ session, player, playerCount }) => <Link href={player.rsvp === "invited" ? `/s/${session.slug}` : `/games/${session.id}`} prefetch={false} key={session.id} style={sessionAccentStyle(session.accentColor)} className="collection-row pressable group flex min-h-20 items-center gap-3 py-4 hover:bg-surface-strong sm:gap-4 sm:px-2"><CalendarBlank aria-hidden className="shrink-0 text-primary" size={20} /><div className="min-w-0 flex-1"><h3 className="truncate font-semibold">{session.title}</h3><p className="mt-1 truncate text-sm text-muted"><time>{formatSessionDate(session.startsAt)}</time> · {formatSessionTime(session.startsAt, session.endsAt)} · {session.venueName}</p></div><span className="score hidden shrink-0 text-sm text-muted sm:block">{playerCount} / {session.capacity}</span><CaretRight aria-hidden className="shrink-0 text-muted transition-transform group-hover:translate-x-0.5" size={16} /></Link>)}</div></section> : null}

    {data.recent.length ? <section aria-labelledby="recent-heading"><h2 id="recent-heading" className="text-lg font-bold">Recent games</h2><div className="mt-3 divide-y divide-line border-y border-line">{data.recent.slice(0, 4).map(({ session, playerCount }) => <Link href={`/s/${session.slug}`} prefetch={false} key={session.id} style={sessionAccentStyle(session.accentColor)} className="pressable flex min-h-20 items-center gap-4 py-4 hover:bg-surface-strong sm:px-2"><MapPin className="text-primary" size={19} /><div className="flex-1"><h3 className="font-semibold">{session.title}</h3><p className="mt-1 text-sm text-muted">{formatSessionDate(session.startsAt)} · {session.venueName} · {playerCount} players</p></div><CaretRight className="text-muted" size={16} /></Link>)}</div></section> : null}
  </div>;
}
