...
const sections = [
  { title: "Getting started", items: [
    ...
    ["How do I know a game is ready?", "Hosts see a short readiness check for the roster, court booking, and payment collection. It guides setup without turning the session into a complicated checklist."],
    ["Can I replay the application tour?", "Yes. Choose Replay tour at the top of Help Center. Relay will point out the real navigation controls again without changing your profile or games."],
  ]},
  { title: "Invites and players", items: [
    ...
    ["When should I create a group?", "Start with a standalone game. After the same crew plays together, save them as a group to reuse members, see upcoming games, and keep shared history together."],
  ]},
  { title: "Chat and notifications", items: [
    ["Who can use session chat?", "Anyone with the shared link can read the session conversation. Joined account players and guest players can send messages, react, and share one photo at a time."],
    ["Does chat update live?", "Yes. New messages, photos, and reactions update while the session chat is open. If the connection pauses, Relay shows the connection state and catches up after reconnecting."],
    ["What appears in notifications?", "Relay collects useful game activity such as invitations, join requests, waitlist movement, booking changes, payment updates, and court assignments. Open a notification to mark it read and go directly to the relevant part of the game."],
    ["How do I clear unread notifications?", "Open Notifications and choose Mark all read, or open an individual update. Use the All and Unread views to find what still needs attention."],
  ]},
  ...
  { title: "Profiles, privacy, and history", items: [
    ...
    ["Who can see a shared session?", "Anyone with the link can view the plan, roster, scores, and session conversation. Joining is required to send chat messages or see an assigned payment. Host controls and payment review stay private."],
    ["What is the difference between a guest and an account player?", "Both can join, chat, view assignments, and manage their own payment from the shared link. An account player also keeps profile history across sessions; a guest remains attached only to that game and device."],
    ["What happens after a session ends?", "The session keeps its plan, players, results, standings, and shared memories together. Play Again copies the reusable setup without copying RSVPs, payments, scores, or photos."],
  ]},
] as const;
