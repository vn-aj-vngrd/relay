...
export default async function VenuesPage() {
  await requireUser();
  const venues = await getCebuVenues();
        ...
        <div className="max-w-2xl">
          <div className="flex items-center gap-2">
            ...
            <IconTooltip
              ...
              label="Cebu pilot: Court Finder currently covers Cebu only. Confirm details with the venue before booking."
            >
              <button
                type="button"
          ...
          <p className="mt-2 text-pretty leading-6 text-muted">Find a nearby court and add it to your game.</p>
        </div>
        <ButtonLink href="/venues/suggest" variant="secondary" className="w-fit">
          <Plus aria-hidden size={16} /> Suggest a court
        </ButtonLink>
      </header>
      <CourtFinder venues={venues} />
    </div>
  );
}
