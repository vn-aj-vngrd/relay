...
export default async function GroupsPage() {
  ...
  upcoming.forEach((session) => { if (session.groupId && !nextByGroup.has(session.groupId)) nextByGroup.set(session.groupId, session); });
  ...
  return <div><header className="flex items-end justify-between gap-4"><div><h1 className="app-title">Groups</h1><p className="mt-2 max-w-xl text-muted">Regular crews, faster invites, and game-night history.</p></div><span className="hidden sm:block"><ButtonLink href="/groups/new"><Plus aria-hidden size={16} />Create group</ButtonLink></span></header><GroupCollection items={items} /></div>;
}
