...
export default async function ProfilePage({ params }: { params: Promise<{ username: string }> }) {
  ...
  return <div className="mx-auto max-w-3xl"><header className="flex items-center gap-4 border-b border-line pb-7"><Avatar name={profile.name} size="lg" /><div className="flex-1"><h1 className="text-2xl font-bold tracking-[-0.03em]">{profile.name}</h1>{profile.city ? <p className="mt-1 flex items-center gap-1 text-sm text-muted"><MapPin size={14} />{profile.city}{profile.dominantHand ? ` · ${profile.dominantHand}-handed` : ""}</p> : <p className="mt-1 text-sm text-muted">@{profile.username}</p>}</div></header><section className="grid grid-cols-3 border-b border-line py-6 text-center"><div><strong className="score block text-2xl">{sessionCount}</strong><span className="text-sm text-muted">Sessions</span></div><div><strong className="score block text-2xl">{participation.length}</strong><span className="text-sm text-muted">Matches</span></div><div><strong className="score block text-2xl">{wins}</strong><span className="text-sm text-muted">Wins</span></div></section><section className="py-8"><h2 className="text-lg font-bold">Recent sessions</h2>{recent.length ? <ul className="mt-3 divide-y divide-line border-y border-line">{recent.map(({ session }) => <li key={session.id} className="flex items-center justify-between py-4"><div><p className="font-semibold">{session.title}</p><p className="mt-1 text-sm text-muted">{formatSessionDate(session.startsAt)} · {session.venueName}</p></div></li>)}</ul> : <p className="mt-3 border-y border-line py-7 text-sm text-muted">No sessions yet. The first one will show up here.</p>}</section>{viewer?.id === profile.userId ? <form action={signOut} className="border-t border-line pt-6"><Button variant="secondary">Sign out</Button></form> : null}</div>;
}
