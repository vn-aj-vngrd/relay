...
export default async function ProfilePage({ params }: { params: Promise<{ username: string }> }) {
  ...
  return <div className="mx-auto max-w-3xl">
    <header className="flex items-center gap-4 pb-7"><Avatar name={profile.name} size="lg" /><div className="min-w-0 flex-1"><h1 className="truncate text-[1.75rem] font-[720] tracking-[-0.025em]">{profile.name}</h1>{profile.city ? <p className="mt-1 flex items-center gap-1.5 text-sm text-muted"><MapPin size={15} />{profile.city}{profile.dominantHand ? ` · ${profile.dominantHand}-handed` : ""}</p> : <p className="mt-1 text-sm text-muted">@{profile.username}</p>}</div></header>

    <section aria-label="Playing history" className="grid grid-cols-3 border-y border-line py-5 text-center"><div><strong className="score block text-2xl">{sessionCount}</strong><span className="text-xs font-medium text-muted sm:text-sm">Sessions</span></div><div className="border-x border-line"><strong className="score block text-2xl">{participation.length}</strong><span className="text-xs font-medium text-muted sm:text-sm">Matches</span></div><div><strong className="score block text-2xl">{wins}</strong><span className="text-xs font-medium text-muted sm:text-sm">Wins</span></div></section>
    <p className="pt-3 text-center text-xs leading-5 text-muted">For fun, not a competitive rating.</p>

    <section className="py-10" aria-labelledby="recent-title"><div className="mb-3 flex items-end justify-between"><div><h2 id="recent-title" className="text-lg font-[680]">Recent sessions</h2><p className="mt-1 text-sm text-muted">Games you played with friends.</p></div></div>{recent.length ? <ul className="divide-y divide-line border-y border-line">{recent.map(({ session }) => <li key={session.id}><Link href={`/games/${session.id}`} prefetch={false} className="pressable flex min-h-20 items-center gap-4 py-4 hover:bg-surface-strong sm:px-2"><time className="score w-20 shrink-0 text-xs font-semibold text-primary">{formatSessionDate(session.startsAt)}</time><div className="min-w-0 flex-1"><p className="truncate font-semibold">{session.title}</p><p className="mt-1 truncate text-sm text-muted">{session.venueName}</p></div><ChevronRight aria-hidden className="text-muted" size={19} /></Link></li>)}</ul> : <div className="border-y border-line py-7"><p className="font-semibold">No sessions yet</p><p className="mt-1 text-sm text-muted">Your first game will show up here.</p></div>}</section>

    {ownProfile ? <section aria-labelledby="settings-title" className="pb-4"><h2 id="settings-title" className="mb-3 text-lg font-[680]">Settings</h2><div className="divide-y divide-line border-y border-line"><ThemeToggle showLabel /><form action={signOut} className="py-2"><Button variant="quiet" className="w-full justify-start px-3">Sign out</Button></form></div></section> : null}
  </div>;
}
