...
export default async function GameOverviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (membership?.rsvp === "pending") return <div className="mx-auto w-full max-w-4xl"><p className="sport-label text-primary">{formatSessionDateLong(session.startsAt).toUpperCase()}</p><h1 className="mt-2 app-title">{session.title}</h1><section className="mt-8 border-y border-line py-10 text-center"><HourglassMedium aria-hidden size={26} className="mx-auto text-warning" /><h2 className="mt-4 text-xl font-bold">Waiting for host approval</h2><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-muted">Your request is with the host. Once approved, this page unlocks the roster, chat, payments, Play, and live scores.</p><div className="mt-6"><ButtonLink href={`/s/${session.slug}`} variant="secondary">View shared game</ButtonLink></div></section></div>;
