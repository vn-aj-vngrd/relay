...
export default async function GameOverviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div><div className="mb-5 flex flex-wrap items-start justify-between gap-4"><div><p className="score mb-2 text-sm font-semibold text-primary">{formatSessionDateLong(session.startsAt).toUpperCase()}</p><h1 className="text-[28px] font-bold tracking-[-0.035em] sm:text-4xl">{session.title}</h1><p className="mt-2 text-muted">{isHost ? "Hosted by you" : "You’re going"}</p></div><ShareButton url={`/s/${session.slug}`} title={session.title} /></div><SessionNav id={session.id} />
