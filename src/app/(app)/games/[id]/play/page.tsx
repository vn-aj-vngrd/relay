...
export default async function PlayPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const waitingPairs = data.pairs.map((pair) => ({ ...pair, players: pair.members.flatMap((id) => waitingById.get(id) ? [waitingById.get(id)!] : []) })).filter((pair) => pair.players.length === 2).toSorted((left, right) => Math.min(...left.players.map((item) => item.queue.position)) - Math.min(...right.players.map((item) => item.queue.position)));
  const goingCount = data.roster.filter(({ player }) => player.rsvp === "going").length;
  const roundMode = data.session.rotationMode === "random" || data.session.rotationMode === "king_of_court" || data.session.rotationMode === "round_robin";
  ...
  return <><GamePageIntro title="Play" description="Court assignments, scores, partner rotations, and who plays next." action={<div className="flex items-center gap-2">{data.session.status === "live" ? <span className="inline-flex items-center gap-1.5 text-sm font-semibold text-live"><Broadcast aria-hidden size={17} />Play in progress</span> : null}<RealtimeRefresh sessionId={data.session.id} /></div>} />
    {data.session.status !== "live" ? <section className="mx-auto w-full max-w-4xl py-10 sm:py-14">
      ...
      <section>
        ...
        })}</div> : <div className="border-y border-line py-10"><h3 className="font-bold">{roundRobinComplete ? "Round robin complete" : data.completedMatchCount ? "Ready for what’s next" : "Courts are open"}</h3><p className="mt-2 text-sm text-muted">{roundRobinComplete ? "Every pair has played each other once." : waiting.length < 4 ? `Waiting for ${4 - waiting.length} more ${4 - waiting.length === 1 ? "player" : "players"}.` : roundMode ? "Every court is ready for the next round." : "The next four players are ready."}</p>{isHost && canStartRotation ? <form action={createQueueMatch} className="mt-5"><input type="hidden" name="sessionId" value={data.session.id} /><SubmitButton pendingLabel={roundMode ? "Starting round…" : "Starting match…"}><Shuffle size={17} />{rotationLabel}</SubmitButton></form> : null}</div>}
      </section>

      <aside>
        <div className="flex items-end justify-between"><div><h2 className="text-lg font-bold">{data.pairs.length ? "Team queue" : roundMode ? "Waiting & resting" : "Paddle stack"}</h2><p className="mt-1 text-sm text-muted">{data.session.rotationMode === "round_robin" ? "Fixed pairs · next unplayed matchup" : data.pairs.length ? "Pairs stay together · longest waiting first" : roundMode ? "Fair rests are prioritized next round" : "Up next · longest waiting first"}</p></div></div>
        {data.pairs.length ? waitingPairs.length ? <ol className="mt-3 divide-y divide-line border-y border-line">{waitingPairs.map((pair, index) => { const names = pair.players.map(({ player, profile }) => playerName(player, profile)); return <li key={pair.id} className="flex min-h-16 items-center gap-3 py-2"><span className="score w-5 text-center text-sm font-bold text-muted">{index + 1}</span><span className="flex -space-x-2">{pair.players.map(({ player, profile }, playerIndex) => <Avatar key={player.id} name={names[playerIndex]} imageUrl={profileAvatarUrl(profile?.avatarPath)} index={playerIndex} size="sm" />)}</span><span className="min-w-0 flex-1 truncate text-sm font-semibold">{names.join(" + ")}</span>{isHost ? <DotsSixVertical aria-label={`Move ${names.join(" and ")}`} className="text-muted" size={18} /> : null}</li>; })}</ol> : <p className="mt-3 border-y border-line py-7 text-sm text-muted">Every pair is currently playing.</p> : waiting.length ? <ol className="mt-3 divide-y divide-line border-y border-line">{waiting.map(({ queue, player, profile }, index) => {
