...
import { getCourtListingBySlug, getCourtListings } from "@/features/venues/directory";
import { VenueSubmissionForm } from "@/features/venues/venue-submission-form";

export default async function SuggestCourtPage({ searchParams }: { searchParams: Promise<{ court?: string }> }) {
    ...
    <div>
      <header className="mb-8 max-w-2xl border-b border-line pb-7">
        ...
        <p className="mt-2 text-pretty leading-6 text-muted">
          Add a missing court or suggest a correction to an existing listing. Relay reviews every source before public
          information changes.
        </p>
      </header>
      <VenueSubmissionForm courts={courts} initialVenue={initialVenue ?? undefined} />
    </div>
  );
}
