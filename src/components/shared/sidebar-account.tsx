...
function SignOutButton() {
  ...
  return (
    <button
      role="menuitem"
      disabled={pending}
      className="flex min-h-9 w-full items-center gap-2 rounded-md px-2 text-[13px] text-muted hover:bg-surface-strong hover:text-ink disabled:opacity-50"
    >
      {pending ? <ButtonSpinner /> : <SignOut size={17} />}
  ...
  return (
    <div ref={root} data-tour="profile" className="sidebar-account relative">
      ...
      {open ? (
        <div
          role="menu"
          className="sidebar-account-menu absolute bottom-11 left-0 z-40 w-full min-w-52 rounded-lg border border-line bg-surface p-1 shadow-[0_4px_8px_oklch(0.1_0.01_275/.12)]"
        >
          <div className="border-b border-line px-2 py-2">
            <p className="truncate text-sm font-medium">{name}</p>
            <p className="mt-0.5 truncate text-xs text-muted">@{username}</p>
          </div>
          <Link
            role="menuitem"
            href={`/profile/${username}`}
            onClick={() => setOpen(false)}
            className="mt-1 flex min-h-9 items-center gap-2 rounded-md px-2 text-sm text-muted hover:bg-surface-strong hover:text-ink"
          >
            <UserCircle size={17} />
            Profile
          </Link>
