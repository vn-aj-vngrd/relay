...
export function RelayMark({ inverse = false, className = "h-7 w-7" }: { inverse?: boolean; className?: string }) {
  ...
  const line = inverse ? "var(--ink)" : "var(--surface)";
...
export function Brand({ inverse = false }: { inverse?: boolean }) {
  return <Link href="/" prefetch={false} className={`-mx-1 inline-flex min-h-11 items-center gap-2 px-1 font-semibold tracking-[-0.02em] ${inverse ? "text-white" : "text-ink"}`} aria-label="Relay home"><RelayMark inverse={inverse} /><span className="text-[16px]">Relay</span></Link>;
}
