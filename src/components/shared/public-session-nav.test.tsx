...
describe("PublicSessionNav", () => {
  it("uses the same session destinations as the account workspace", () => {
    ...
    expect(navigation.querySelector(".max-w-6xl > .border-b")).toHaveClass(
      "border-line"
    );
  });
  ...
  it("scopes the active state to the session accent", () => {
    ...
    expect(gameTitle.parentElement).toContainElement(
      screen.getByRole("link", { name: "Relay home" })
    );
    ...
    expect(
      container.querySelector<HTMLElement>("header.app-chrome")
    ).not.toHaveClass("sticky");
    expect(
      screen.getByRole("navigation", { name: "Game navigation" }).parentElement
    ).toHaveClass("sticky", "top-0");
    expect(
      container
        .querySelector<HTMLElement>(".app-chrome")
