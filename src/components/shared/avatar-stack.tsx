...
export function AvatarStack({
  ...
  return (
    <div role="group" className="flex items-center" aria-label={`${total ?? names.length} players`}>
      ...
      {(total ?? names.length) > names.length ? (
        <span className="-ml-2 inline-flex h-10 min-w-10 items-center justify-center rounded-full border-2 border-canvas bg-surface-strong px-2 text-xs font-bold">
          +{(total ?? names.length) - names.length}
        </span>
      ) : null}
    </div>
