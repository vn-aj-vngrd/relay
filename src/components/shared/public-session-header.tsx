...
export function PublicSessionHeader({ slug, signedIn, gameHref, accentColor }: { slug: string; signedIn: boolean; gameHref?: string; accentColor?: string | null }) {
  ...
  return <header className="app-chrome sticky top-0 z-20" style={sessionAccentStyle(accentColor)}>
    <div className="mx-auto flex h-14 w-full max-w-4xl items-center justify-between gap-4 px-4 sm:px-6">
      <Brand />
      <ButtonLink href={destination} variant={signedIn ? "primary" : "secondary"}>{label}</ButtonLink>
    </div>
