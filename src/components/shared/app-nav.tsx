...
export function AppNav({ username, mode }: { username: string; mode: "sidebar" | "mobile" }) {
  ...
  if (mode === "sidebar") {
    ...
    return <nav aria-label="Main navigation"><ul className="space-y-0.5">{desktopItems.map(({ href, label, icon: Icon, primary }) => {
      ...
      return <li key={href} className={primary ? "mb-3" : undefined}><Link href={href} data-tour={label === "Create game" ? "create" : label.toLowerCase()} prefetch={false} aria-current={active ? "page" : undefined} className={`sidebar-row pressable flex min-h-9 items-center gap-2.5 rounded-md px-2 text-[14px] font-medium ${primary ? "bg-primary text-white hover:bg-primary-hover" : active ? "bg-surface-strong text-ink" : "text-muted hover:bg-surface-strong/70 hover:text-ink"}`}><Icon aria-hidden size={18} weight={active || primary ? "fill" : "regular"} className={primary ? "text-white" : active ? "text-primary" : "text-muted"} /><span>{label}</span></Link></li>;
    })}</ul></nav>;
  }
