...
export function ThemeToggle({ inverse = false }: { inverse?: boolean }) {
  ...
  return (
    <button
      ...
      className={`pressable grid h-11 w-11 place-items-center rounded-xl border ${inverse ? "border-white/15 text-white hover:bg-white/10" : "border-transparent text-muted hover:bg-surface-strong hover:text-ink"}`}
    >
