...
const triggerClass =
  "mt-1.5 flex h-11 w-full items-center gap-2 rounded-lg border bg-surface px-3 text-left text-[15px] text-ink focus:outline-none";
...
export function DatePickerField({
  ...
  return (
    <div ref={root} className="relative min-w-0">
      <label className="block text-sm font-[650]" htmlFor={id}>
        {label}
      </label>
      ...
      <button
        ...
        className={`${triggerClass} ${error ? "border-danger ring-2 ring-danger/10" : "border-line focus:border-primary focus:ring-2 focus:ring-primary/15"}`}
      >
        <CalendarBlank aria-hidden size={17} className="shrink-0 text-muted" />
        <span className={`flex-1 ${selected ? "" : "text-muted"}`}>
          {selected ? dateFormatter.format(selected) : "Choose a date"}
        </span>
