...
describe("date and time pickers", () => {
  ...
  it("disables dates before the creation boundary", () => {
    render(<DatePickerField id="date" label="Date" defaultValue="2030-09-06" minValue="2030-09-06" />);
    fireEvent.click(screen.getByRole("button", { name: "Date" }));
    expect(screen.getByRole("button", { name: "Thursday, September 5, 2030" })).toBeDisabled();
    expect(screen.getByRole("button", { name: "Friday, September 6, 2030" })).toBeEnabled();
  });
  ...
  it("offers end times after the start and start times before the end", () => {
    const { unmount } = render(<TimePickerField id="end" label="End time" afterValue="20:00" />);
    ...
    unmount();
    render(<TimePickerField id="start" label="Start time" beforeValue="20:00" />);
    fireEvent.click(screen.getByRole("button", { name: "Start time" }));
