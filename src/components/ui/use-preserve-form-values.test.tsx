...
function InvalidForm() {
  const [state, action] = useActionState(
    async (): Promise<{ error?: string; fieldErrors?: Record<string, string[]> }> => ({
  ...
  return (
    <form noValidate action={action} onSubmitCapture={preserveValues}>
      ...
      <div>
        <textarea id="long-answer" name="answer" aria-invalid={Boolean(state.fieldErrors?.answer)} />
        {state.fieldErrors?.answer ? <p className="text-danger">{state.fieldErrors.answer[0]}</p> : null}
      </div>
...
describe("usePreserveFormValuesOnError", () => {
  it("restores uncontrolled values after a form action returns an error", async () => {
    ...
    expect(screen.getByLabelText("Long answer")).toHaveAttribute("aria-invalid", "false");
  });
});
