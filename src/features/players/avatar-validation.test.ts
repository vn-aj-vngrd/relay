...
describe("validateAvatarFile", () => {
  it("accepts a real supported image signature", async () => {
    ...
    await expect(validateAvatarFile(png)).resolves.toMatchObject({
      extension: "png",
      file: png,
    });
  });
  ...
  it("rejects unsupported and spoofed files", async () => {
    ...
    const spoofed = new File(["not an image"], "avatar.png", {
      type: "image/png",
    });
    await expect(validateAvatarFile(text)).resolves.toEqual({
  ...
  it("enforces the storage bucket size limit before upload", async () => {
    ...
    await expect(validateAvatarFile(large)).resolves.toEqual({
