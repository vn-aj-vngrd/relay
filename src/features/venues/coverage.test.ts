...
describe("Court directory coverage", () => {
  it("contains locations across the Philippines but excludes other countries", () => {
    expect(
      courtDirectoryCoverage.contains({
        latitude: 14.5995,
        longitude: 120.9842,
      })
    ).toBe(true);
    ...
    expect(
      courtDirectoryCoverage.contains({ latitude: 7.0731, longitude: 125.6128 })
    ).toBe(true);
    expect(
      courtDirectoryCoverage.contains({
        latitude: 35.6762,
  ...
  it("returns field-specific publishing issues", () => {
    expect(
      courtDirectoryCoverage.validatePublishingCoordinate({
        latitude: "",
        longitude: "",
      })
    ).toEqual([
  ...
  it("allows map tiles across the supported country", () => {
    for (const [longitude, latitude] of [
      ...
      expect(courtDirectoryCoverage.allowsTile({ zoom: 12, ...tile })).toBe(
        true
      );
    }
  });
  ...
  it("rejects distant, malformed, and excessive tile requests", () => {
    ...
    expect(courtDirectoryCoverage.allowsTile({ zoom: 12, ...tokyo })).toBe(
      false
    );
    expect(courtDirectoryCoverage.allowsTile({ zoom: 4, x: 0, y: 0 })).toBe(
      false
    );
    expect(courtDirectoryCoverage.allowsTile({ zoom: 19, x: 0, y: 0 })).toBe(
      false
    );
    expect(courtDirectoryCoverage.allowsTile({ zoom: 12, x: -1, y: 0 })).toBe(
      false
    );
  });
