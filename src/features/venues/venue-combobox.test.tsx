...
describe("VenueCombobox", () => {
  it("shows Relay courts immediately without a location provider request", () => {
    ...
    expect(
      screen.getByRole("option", { name: /Central Pickle/ })
    ).toBeVisible();
  ...
  it("filters local courts immediately and saves the selected address", () => {
    ...
    expect(
      screen.queryByRole("option", { name: /South Court/ })
    ).not.toBeInTheDocument();

    fireEvent.click(screen.getByRole("option", { name: /Central Pickle/ }));
    expect(input).toHaveValue("Central Pickle");
    expect(
      document.querySelector<HTMLInputElement>('input[name="venueId"]')
    ).toHaveValue("venue-1");
    expect(
      document.querySelector<HTMLInputElement>('input[name="venueAddress"]')
    ).toHaveValue("Greenfield District, Mandaluyong");
  ...
  it("still allows a manually entered court when the directory has no match", () => {
    ...
    fireEvent.change(input, {
      target: { value: "Private neighborhood court" },
    });
