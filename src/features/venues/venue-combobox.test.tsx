...
describe("VenueCombobox", () => {
  ...
  it("debounces Philippine venue suggestions and saves the selected address", async () => {
    ...
    render(<><label htmlFor="venue">Venue</label><VenueCombobox /></>);
    ...
    expect(fetchMock).toHaveBeenCalledWith("/api/venues/search?q=Central", expect.objectContaining({ signal: expect.any(AbortSignal) }));

    fireEvent.click(screen.getByRole("option", { name: /Central Pickle/ }));
    expect(input).toHaveValue("Central Pickle");
    expect(document.querySelector<HTMLInputElement>('input[name="venueAddress"]')).toHaveValue("Greenfield District, Mandaluyong, Philippines");
    expect(screen.queryByRole("listbox")).not.toBeInTheDocument();
  });
});
