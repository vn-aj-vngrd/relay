...
type OperatingHourField =
  `${(typeof courtDays)[number]["key"]}${"Open" | "Close"}`;

const optionalCourtTime = z.union([
  z.literal(""),
  z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/),
]);
const optionalPrice = z.union([
  z.literal(""),
...
function validateStructuredDetails(
  ...
  if (
    value.priceStatus === "paid" &&
    (value.priceAmount === "" || value.priceUnit === "")
  )
    context.addIssue({
      code: "custom",
  ...
  if (
    ...
    context.addIssue({
      code: "custom",
      path: ["priceAmount"],
      message: "Starting price must be above ₱0.",
    });
  if (
    value.priceMax !== "" &&
    (value.priceAmount === "" || value.priceMax < value.priceAmount)
  )
    context.addIssue({
      code: "custom",
      path: ["priceMax"],
...
export const venueSubmissionSchema = z
  .object({
    ...
    changedFields: z
      .array(z.enum(venueChangeGroups))
      .max(venueChangeGroups.length),
    name: z
      .string()
      .trim()
      .min(2, "Add the court name.")
      .max(120, "Keep the name under 120 characters."),
    address: z
      .string()
      .trim()
      .min(5, "Add a useful street or neighborhood address.")
      .max(240),
  ...
  .superRefine((value, context) => {
    ...
    if (value.requestType === "create" && value.city.length < 2)
      context.addIssue({
        code: "custom",
        path: ["city"],
        message: "Add the Philippine city or municipality.",
      });
    if (value.requestType === "update" && value.venueId === "")
      context.addIssue({
        code: "custom",
        path: ["venueId"],
        message: "Choose the court you want to update.",
      });
    if (value.requestType === "update" && value.changedFields.length === 0)
      context.addIssue({
        code: "custom",
        path: ["changedFields"],
        message: "Choose at least one detail to update.",
      });
  });
...
export const adminVenueSchema = z
  ...
  .superRefine((value, context) => {
    ...
    for (const issue of courtDirectoryCoverage.validatePublishingCoordinate(
      value
    ))
      context.addIssue({
        code: "custom",
