...
describe("structured court details", () => {
  it("formats structured prices and ranges in Philippine pesos", () => {
    ...
    expect(formatCourtPrice({ priceStatus: "free", priceAmountCents: 0, priceMaxCents: null, priceUnit: null })).toBe(
      "Free",
    );
    expect(
      formatCourtPrice({ priceStatus: "contact", priceAmountCents: null, priceMaxCents: null, priceUnit: null }),
    ).toBe("Ask the court");
  });

  it("stores only fields allowed by the selected pricing status", () => {
    expect(toCourtPriceStorage({ priceStatus: "free", priceAmount: "", priceMax: "", priceUnit: "" })).toEqual({
      priceStatus: "free",
      priceAmountCents: 0,
      priceMaxCents: null,
      priceUnit: null,
    });
    expect(toCourtPriceStorage({ priceStatus: "paid", priceAmount: 450.5, priceMax: 600, priceUnit: "hour" })).toEqual({
      priceStatus: "paid",
  ...
  it("builds filterable Philippine operating hours", () => {
    ...
    expect(formatCourtOperatingHours(operatingHours)).toBe("Mon 6:00 AM–10:00 PM; Sat 8:00 AM–8:00 PM");
    expect(isCourtOpenAt(operatingHours, new Date("2026-09-07T04:00:00.000Z"))).toBe(true);
