...
describe("SessionRecap states", () => {
  ...
  it("shows provisional facts while play is underway", () => {
    ...
    expect(
      screen.getByText(/1 completed match is included so far/)
    ).toBeInTheDocument();
  ...
  it("places optional feedback before the existing continuation actions", () => {
    renderRecap(
      ...
      <p>How did this game go?</p>
    );

    const feedback = screen.getByText("How did this game go?");
    const continuation = screen.getByRole("heading", {
      name: "Keep this crew moving.",
    });
    expect(
      feedback.compareDocumentPosition(continuation) &
  ...
  it("consolidates the host’s post-game actions", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Keep this crew moving." })
    ).toBeInTheDocument();
    expect(screen.getByRole("link", { name: /Play again/ })).toHaveAttribute(
      "href",
      "/games/new?from=session"
    );
    expect(
      screen.getByRole("link", { name: /Save this crew/ })
    ).toHaveAttribute("href", "/groups/new?from=session");
    expect(screen.getByRole("link", { name: /Share recap/ })).toHaveAttribute(
      "href",
      "/games/session/story"
    );
    expect(
      screen.queryByRole("link", { name: /Open story/ })
    ).not.toBeInTheDocument();
  });
  ...
  it("hides Save this crew when the completed game already belongs to a group", () => {
    ...
    expect(
      screen.queryByRole("link", { name: /Save this crew/ })
    ).not.toBeInTheDocument();
    expect(
      screen.getByRole("link", { name: /Play again/ })
    ).toBeInTheDocument();
  });
});
