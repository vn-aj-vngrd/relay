...
export async function getSessionRecapData(sessionId: string) {
  ...
  const members = matchIds.length
    ? await db
        ...
        .innerJoin(
          sessionPlayers,
          eq(sessionPlayers.id, matchPlayers.sessionPlayerId)
        )
  ...
  const recap = buildSessionRecap(
    sessionMatches.map((match) => ({
      ...
      teamA: members
        .filter(
          ({ matchPlayer }) =>
            matchPlayer.matchId === match.id && matchPlayer.team === "A"
        )
      ...
      teamB: members
        .filter(
          ({ matchPlayer }) =>
            matchPlayer.matchId === match.id && matchPlayer.team === "B"
        )
        .sort((a, b) => a.matchPlayer.position - b.matchPlayer.position)
        .map(({ player }) => player.id),
...
export async function getSessionRecap(sessionId: string) {
  const [recap, memory] = await Promise.all([
    getSessionRecapData(sessionId),
