...
describe("SetupWizard", () => {
  it("sequences identity, optional profile details, confirmation, and completion", async () => {
    ...
    expect(
      screen.getByRole("heading", { name: "How should players know you?" })
    ).toBeVisible();
    ...
    expect(
      screen.getByRole("heading", { name: "Add more about your game" })
    ).toBeVisible();
    ...
    expect(
      within(confirmation!).getByText("Weekend doubles player")
    ).toBeVisible();
    ...
    expect(
      screen.getByRole("link", { name: "Start product tour" })
    ).toHaveAttribute("href", "/home?tour=1&next=%2Fgames%2Fnew");
  });

  it("prevents invalid identity details before moving forward", () => {
    render(
      <SetupWizard
        initial={{ ...initial, name: "", username: "Bad Name" }}
        next="/games/new"
      />
    );
