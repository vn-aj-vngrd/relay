...
const setupSchema = z.object({
  name: z.string().trim().min(2, "Add the name your friends know you by.").max(60, "Keep your name under 60 characters."),
  username: z.string().trim().toLowerCase().min(3, "Use at least 3 characters.").max(24, "Keep your username under 24 characters.").regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/, "Use lowercase letters, numbers, and single hyphens."),
...
export async function completeProfileSetup(_: OnboardingActionState, formData: FormData): Promise<OnboardingActionState> {
  ...
  if (!parsed.success) return { error: "Check the details marked below.", fieldErrors: parsed.error.flatten().fieldErrors };

  const usernameOwner = await db.query.profiles.findFirst({ where: and(eq(profiles.username, parsed.data.username), ne(profiles.userId, user.id)) });
  if (usernameOwner) return { error: "That username is already taken.", fieldErrors: { username: ["Try another username."] } };
  ...
  try {
    ...
    if (typeof error === "object" && error !== null && "code" in error && error.code === "23505") return { error: "That username is already taken.", fieldErrors: { username: ["Try another username."] } };
