...
describe("admin action validation", () => {
  it("requires a useful reason and trims it", () => {
    ...
    expect(adminReasonSchema.parse("  Duplicate account  ")).toBe(
      "Duplicate account"
    );
  });
  ...
  it("rejects malformed user and session identifiers", () => {
    ...
    expect(
      adminSessionActionSchema.safeParse({
        sessionId: "game",
        reason: "Host request",
      }).success
    ).toBe(false);
  ...
  it("accepts a bounded whole-number signup capacity", () => {
    expect(adminSignupCapacitySchema.parse({ accountCap: "200" })).toEqual({
      accountCap: 200,
    });
    expect(
      adminSignupCapacitySchema.safeParse({ accountCap: "0" }).success
    ).toBe(false);
    expect(
      adminSignupCapacitySchema.safeParse({ accountCap: "2.5" }).success
    ).toBe(false);
    expect(
      adminSignupCapacitySchema.safeParse({ accountCap: "50001" }).success
    ).toBe(false);
  });
