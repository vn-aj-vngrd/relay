...
vi.mock("./actions", () => ({ updateSignupCapacityAction: vi.fn(async () => ({})) }));
...
describe("SignupCapacityControl", () => {
  it("shows the current limit and remaining beta places", () => {
    ...
    expect(screen.getByRole("spinbutton", { name: "Maximum accounts" })).toHaveValue(200);
    expect(screen.getByText("25 places remaining")).toBeVisible();
    expect(screen.getByRole("button", { name: "Save account limit" })).toBeEnabled();
  });
