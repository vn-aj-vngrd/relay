...
const notificationCursorSchema = z.object({ at: z.iso.datetime({ offset: true }), id: z.uuid() });
...
export function parseNotificationCursor(value: string | null): NotificationCursor | null {
  if (!value) return null;
  try {
    const parsed = notificationCursorSchema.safeParse(JSON.parse(Buffer.from(value, "base64url").toString("utf8")));
