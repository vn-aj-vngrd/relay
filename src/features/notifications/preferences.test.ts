...
describe("notification delivery preferences", () => {
  it("uses one category policy across email and push", () => {
    ...
    expect(channelAllowsNotification("email", "moved_from_waitlist")).toBe(true);
    expect(channelAllowsNotification("email", "session_starting_soon")).toBe(false);
    expect(channelAllowsNotification("push", "session_starting_soon")).toBe(true);
  ...
  it("honors each reminder timing choice", () => {
    ...
    expect(reminderTimingEnabled("session_starting_soon", true, false)).toBe(false);
  ...
  it("handles quiet hours that cross midnight in the user’s time zone", () => {
    ...
    expect(isWithinQuietHours(atElevenPmManila, "Asia/Manila", "22:00", "07:00")).toBe(true);
    expect(isWithinQuietHours(atNoonManila, "Asia/Manila", "22:00", "07:00")).toBe(false);
  });
});
