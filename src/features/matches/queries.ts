...
import { courts, matches, matchPlayers, profiles, sessionPairMembers, sessionPairs, sessionPlayers, sessionQueue } from "@/db/schema";
...
async function getLiveDetails(sessionId: string) {
  ...
  const pairs = pairIds.map((id) => ({ id, position: pairRows.find((row) => row.id === id)?.position ?? 0, members: pairRows.filter((row) => row.id === id).map((row) => row.sessionPlayerId) }));
  return { courts: sessionCourts, activeMatches: activeMatches.map((match) => ({ ...match, players: players.filter((item) => item.matchPlayer.matchId === match.id) })), completedMatchCount, queue, pairs, standings };
}
