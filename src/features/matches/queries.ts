...
export async function getLiveSession(sessionId: string, userId: string) {
  const base = await getSessionForUser(sessionId, userId);
  if (!base) return null;
  ...
  return { ...base, courts: sessionCourts, activeMatches: activeMatches.map((match) => ({ ...match, players: players.filter((item) => item.matchPlayer.matchId === match.id) })), queue, standings };
}
