...
export const helpArticles: readonly HelpArticle[] = [
  {
    ...
    troubleshooting: [
      ...
      "If a reply fails, its error appears in the conversation with Retry underneath. Any partial answer remains visible. Retry regenerates the latest response without sending your question twice. Stopped or interrupted work is labeled in its activity summary. A response still running in another tab shows a waiting message, then its saved reply appears in the open chat. If syncing fails, reopen the chat from History. History updates loaded Chats and Archived rows when another tab changes them; if another tab deletes your open chat, Agent returns to Your chats. A request rejected before Agent starts does not keep New chat blocked; follow the error guidance and try again when available.",
    ],
