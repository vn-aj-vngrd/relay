...
function syncSearchUrl(url: string) {
      ...
      {Array.from({ length: 6 }, (_, index) => (
        <div key={index} className="flex min-h-16 items-center gap-3 py-3">
          ...
          <span className="flex-1">
            <span className="block h-4 w-40 max-w-[55%] animate-pulse rounded bg-surface-strong" />
            <span className="mt-2 block h-3 w-64 max-w-[75%] animate-pulse rounded bg-surface-strong" />
          </span>
        </div>
      ))}
    </div>
...
export function GlobalSearch({
  ...
  useEffect(() => {
    ...
    if (debouncedQuery)
      queueMicrotask(() => {
        if (!cancelled) void fetchPage(0, false);
      });
    return () => {
      cancelled = true;
      requestRef.current?.abort();
  ...
  const loadMore = useCallback(() => {
    if (nextCursor !== null && !loadingMore) void fetchPage(nextCursor, true);
  ...
  function applyRecent(saved: RecentSearch) {
        ...
        {!query.trim() ? (
          recent.length ? (
            <section aria-labelledby="recent-searches">
              <div className="flex items-center justify-between">
                <h2 id="recent-searches" className="text-lg font-bold">
                  Recent searches
                </h2>
                <button
                  type="button"
                  ...
                  className="min-h-9 text-[13px] font-semibold text-muted hover:text-ink"
                >
                  Clear all
                </button>
              </div>
              <ul className="mt-3 divide-y divide-line border-y border-line">
                {recent.map((saved) => (
                  <li key={`${saved.filter}-${saved.query}`} className="flex min-h-14 items-center gap-2">
                    ...
                    <button
                      type="button"
                      onClick={() => removeRecent(saved)}
                      aria-label={`Remove ${saved.query} from recent searches`}
                      className="grid h-10 w-10 place-items-center text-lg text-muted hover:text-ink"
                    >
                      ×
                    </button>
                  </li>
          ...
          <section className="border-y border-line py-9">
            ...
            <button
              ...
              className="mt-4 min-h-9 rounded-lg bg-primary px-3 text-[13px] font-semibold text-white"
            >
              Try again
            </button>
          </section>
            ...
            {grouped.map((section) => (
              <section key={section.type} aria-labelledby={`results-${section.type}`}>
                <div className="mb-2 flex items-center justify-between">
                  ...
                  <button
                    type="button"
                    onClick={() => chooseFilter(section.type)}
                    className="min-h-9 text-[13px] font-semibold text-primary"
                  >
                    See all
                  </button>
                </div>
        ...
        {query.trim() && nextCursor !== null && status === "ready" ? (
          <div ref={sentinelRef} className="flex min-h-20 items-center justify-center">
            <button
              ...
              className="inline-flex min-h-9 items-center gap-2 text-[13px] font-semibold text-muted hover:text-ink disabled:opacity-60"
            >
              {loadingMore ? (
                <>
                  <ButtonSpinner />
