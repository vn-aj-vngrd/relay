...
describe("GlobalSearch", () => {
  it("uses recent searches instead of a minimum-character instruction", async () => {
    localStorage.setItem("relay-recent-searches-v1", JSON.stringify([{ query: "Central Pickle", filter: "venues", savedAt: 1 }]));
    render(<GlobalSearch />);
  ...
  it("searches after one debounced keystroke without submitting a form", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(expect.stringContaining("q=v"), expect.objectContaining({ signal: expect.any(AbortSignal) }));
  });
  ...
  it("loads another bounded page without replacing current results", async () => {
    vi.mocked(fetch).mockImplementation(async (input) => {
      ...
      const item = { id: cursor === "0" ? "one" : "two", type: "players", title: cursor === "0" ? "Van" : "Mika", subtitle: "@player", href: "/profile/player" };
