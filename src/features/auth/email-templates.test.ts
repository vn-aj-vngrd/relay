...
function template(name: string) {
  return readFileSync(join(process.cwd(), "supabase", "templates", name), "utf8");
}

describe("Supabase Auth email templates", () => {
  it.each(Object.entries(templates))("keeps %s branded and preserves its required destination", (name, required) => {
