...
export function sessionActor(input: {
  userId: string;
  hostId: string;
  membership?: { role: "host" | "cohost" | "player"; rsvp: string; leftAt: Date | null } | null;
}): SessionActor {
  if (input.userId === input.hostId) return { userId: input.userId, role: "host" };
  const membership = input.membership;
  const role =
    membership && !membership.leftAt && participatingRsvps.has(membership.rsvp) ? membership.role : undefined;
...
export function can(
  ...
  if (action === "score")
    return actor.role === "host" || actor.role === "cohost" || Boolean(actor.userId && actor.assignedScorer);
  if (["edit", "manage_roster", "confirm_payment"].includes(action))
    return actor.role === "host" || actor.role === "cohost";
  if (action === "create_expense" || action === "complete" || action === "delete") return actor.role === "host";
  return false;
}
