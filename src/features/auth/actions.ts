...
export async function signInWithPassword(formData: FormData) {
  ...
  if (!email.success || !password.success) authError("Enter a valid email and a password with at least 8 characters, including a letter and number.");
  const supabase = await createSupabaseServerClient();
...
export async function createPasswordAccount(formData: FormData) {
  const email = emailSchema.safeParse(formData.get("email"));
  const password = passwordSchema.safeParse(formData.get("password"));
  if (!email.success || !password.success) authError("Enter a valid email and a password with at least 8 characters, including a letter and number.", "/signup");
  const supabase = await createSupabaseServerClient();
  const { data, error } = await supabase.auth.signUp({
    email: email.data,
    password: password.data,
  });
  if (error) authError(error.message === "User already registered" ? "An account already exists for this email. Log in instead." : "We couldn’t create your account. Please try again.", "/signup");
...
export async function setTemporaryPassword(formData: FormData) {
  ...
  if (!password.success) redirect(`/set-password?error=${encodeURIComponent("Use at least 8 characters with a letter and number.")}`);
  ...
  const { error: metadataError } = await admin.auth.admin.updateUserById(user.id, { app_metadata: { force_password_change: false } });
  if (metadataError) redirect(`/set-password?error=${encodeURIComponent("Your password changed, but account setup could not finish. Submit it once more.")}`);
  redirect("/onboarding");
}
