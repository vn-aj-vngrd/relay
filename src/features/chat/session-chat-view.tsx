...
export async function SessionChatView({
  ...
  for (const reaction of reactionRows) {
    reactionCounts.set(
      reaction.messageId,
      (reactionCounts.get(reaction.messageId) ?? 0) + 1
    );
    if (reaction.sessionPlayerId === viewer.playerId)
      ownReactions.add(reaction.messageId);
  }
      ...
      <ChatThread messageCount={rows.length}>
        {rows.length ? (
          rows.map(({ message, player, profile }, index) => {
            ...
            const grouped =
              previous?.sessionPlayerId === message.sessionPlayerId &&
              previous.kind !== "system" &&
              !newDay;
            const groupEnds =
              next?.sessionPlayerId !== message.sessionPlayerId ||
              next.kind === "system";
                  ...
                  >
                    ...
                    >
                      ...
                      <div>
                        ...
                        >
                          <time className="px-1 text-[11px] text-muted">
