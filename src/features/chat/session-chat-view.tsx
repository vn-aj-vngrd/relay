...
export async function SessionChatView({
  ...
  for (const reaction of reactionRows) {
    reactionCounts.set(reaction.messageId, (reactionCounts.get(reaction.messageId) ?? 0) + 1);
    if (reaction.sessionPlayerId === viewer.playerId) ownReactions.add(reaction.messageId);
  }
  ...
  await Promise.all(
    rows.map(async ({ message }) => {
      ...
      const { data } = await supabase.storage.from("chat-images").createSignedUrl(message.imagePath, 3600);
      if (data?.signedUrl) imageUrls.set(message.id, data.signedUrl);
    }),
  );
  ...
  const dayKey = (date: Date) =>
    new Intl.DateTimeFormat("en-CA", { year: "numeric", month: "2-digit", day: "2-digit", timeZone: timezone }).format(
      date,
    );

  return (
    <div className={`flex min-h-0 flex-col overflow-hidden border-t border-line bg-surface sm:px-3 ${className}`}>
      <ChatThread messageCount={rows.length}>
        {rows.length ? (
          rows.map(({ message, player, profile }, index) => {
            ...
            const grouped =
              previous?.sessionPlayerId === message.sessionPlayerId && previous.kind !== "system" && !newDay;
              ...
              <div key={message.id}>
                {newDay ? (
                  <div className="my-5 flex items-center gap-3 text-xs font-medium text-muted">
                    ...
                    <time dateTime={message.createdAt.toISOString()}>{day.format(message.createdAt)}</time>
                    <span className="h-px flex-1 bg-line" />
                  </div>
                ) : null}
                {system ? (
                      ...
                      <div>
                        ...
                        >
                          {imageUrl ? (
                            <ChatPhotoViewer
                              src={imageUrl}
                              alt={message.body ? `Photo: ${message.body}` : `Photo from ${name}`}
                              sender={name}
                            />
                          ) : null}
                          {message.body ? (
                            <p className="break-words px-3.5 py-2.5 text-[15px] leading-6">{message.body}</p>
                          ) : null}
                        </div>
                        <div
                          className={`mt-1 flex min-h-7 items-center gap-1.5 ${own ? "justify-end" : "justify-start"}`}
                        >
          ...
          <div className="py-16 text-center">
            ...
            <p className="mx-auto mt-2 max-w-sm text-sm leading-6 text-muted">
              Share arrival plans, parking tips, payment reminders, or anything the group needs before game time.
            </p>
          </div>
        )}
      </ChatThread>
