...
describe("chat image limits", () => {
  ...
  it("rejects unsupported and oversized uploads", () => {
    expect(
      validateChatImageMetadata({ type: "image/gif", size: 10 }, 1024)
    ).toBe("Use a JPG, PNG, or WebP photo.");
    expect(
      validateChatImageMetadata({ type: "image/jpeg", size: 1025 }, 1024)
    ).toBe("Choose a photo no larger than 1 KB.");
  });
  ...
  it("checks that the declared image type matches a real signature", async () => {
    ...
    await expect(validateChatImageFile(fake, 1024)).resolves.toEqual({
