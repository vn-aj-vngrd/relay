...
export type GroupActionState = { error?: string; fieldErrors?: Record<string, string[]>; values?: Record<string, string> };
...
export async function createGroupAction(_: GroupActionState, formData: FormData): Promise<GroupActionState> {
  ...
  if (parsed.data.sourceSessionId) {
    const source = await db.query.sessions.findFirst({ where: and(eq(sessions.id, parsed.data.sourceSessionId), eq(sessions.hostId, user.id)) });
    ...
    const players = await db.select({ userId: sessionPlayers.userId }).from(sessionPlayers).where(and(eq(sessionPlayers.sessionId, source.id), eq(sessionPlayers.rsvp, "going")));
  ...
  const group = await db.transaction(async (tx) => {
    ...
    await tx.insert(groupMembers).values(memberIds.map((userId) => ({ groupId: created.id, userId, role: userId === user.id ? "owner" as const : "member" as const })));
    if (parsed.data.sourceSessionId) await tx.update(sessions).set({ groupId: created.id, version: sql`${sessions.version} + 1`, updatedAt: new Date() }).where(eq(sessions.id, parsed.data.sourceSessionId));
    return created;
  });
...
export async function addGroupMemberAction(_: GroupActionState, formData: FormData): Promise<GroupActionState> {
  const user = await requireUser();
  const parsed = addGroupMemberSchema.safeParse({ groupId: formData.get("groupId"), username: formData.get("username") });
  if (!parsed.success) return { error: parsed.error.issues[0]?.message ?? "Enter a valid username." };
  const group = await db.query.groups.findFirst({ where: and(eq(groups.id, parsed.data.groupId), eq(groups.ownerId, user.id)) });
  ...
  const existing = await db.query.groupMembers.findFirst({ where: and(eq(groupMembers.groupId, group.id), eq(groupMembers.userId, profile.userId)) });
