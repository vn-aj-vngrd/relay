...
describe("RsvpControl", () => {
  it("keeps guest RSVP primary while leaving existing players a quiet sign-in path", () => {
    ...
    ).toHaveClass("min-h-9", "text-[13px]");
    expect(screen.getByRole("button", { name: "Share game" })).toHaveClass(
      "min-h-9",
      "text-[13px]"
    );
    expect(screen.getByRole("button", { name: "Going" })).toHaveClass(
      "min-h-9",
      "text-[13px]"
    );
    expect(
      screen.getByRole("button", { name: "Playing experience (optional)" })
    ).toBeVisible();
    expect(
      screen.getByRole("link", { name: "Already use Relay? Sign in" })
    ).toHaveAttribute("href", "/login?next=%2Fs%2Ffriends-night");
    expect(
      screen.queryByRole("link", { name: "Keep this game in Relay" })
  ...
  it("offers account claiming only after a guest response is saved", async () => {
    ...
    expect(
      await screen.findByRole("heading", { name: "Your spot is saved." })
    ).toBeVisible();
    expect(
      screen.getByRole("link", { name: "Keep this game in Relay" })
    ).toHaveAttribute("href", "/signup?next=%2Fs%2Ffriends-night");
  ...
  it("uses the account profile experience without allowing a session override", () => {
    ...
    expect(
      screen.queryByRole("button", { name: "Playing experience (optional)" })
    ).not.toBeInTheDocument();
