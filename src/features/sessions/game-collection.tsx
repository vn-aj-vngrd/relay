...
function EmptyCollection({ past }: { past?: boolean }) {
  return <div className="border-y border-line py-8"><p className="font-[650]">{past ? "No game memories yet" : "Nothing scheduled"}</p><p className="mt-1 text-sm text-muted">{past ? "Completed games will stay here with scores and photos." : "Create a game, share the link, and let the roster fill itself."}</p>{!past ? <Link href="/games/new" className="mt-5 inline-flex min-h-9 items-center rounded-lg bg-primary px-3 text-sm font-[650] text-white hover:bg-primary-hover">Create game</Link> : null}</div>;
}
...
function MonthCalendar({ upcoming, past, todayKey, weekStart }: { upcoming: GameCollectionItem[]; past: GameCollectionItem[]; todayKey: string; weekStart: "sunday" | "monday" }) {
  ...
  return <section aria-labelledby="calendar-month"><div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h2 id="calendar-month" className="text-xl font-[680]">{title}</h2><p className="mt-1 text-sm text-muted"><span className="text-live">● Live</span><span className="mx-2">·</span><span className="text-primary">● Upcoming</span><span className="mx-2">·</span>Past</p></div><div className="flex self-end gap-1 sm:self-auto"><button type="button" onClick={() => changeMonth(-1)} aria-label="Previous month" className="pressable grid h-9 w-9 place-items-center rounded-md text-muted hover:bg-surface-strong hover:text-ink"><CaretLeft aria-hidden size={16} /></button><button type="button" onClick={() => setMonthKey(todayKey.slice(0, 7))} className="pressable min-h-9 rounded-md px-2.5 text-[13px] font-[650] text-muted hover:bg-surface-strong hover:text-ink">Today</button><button type="button" onClick={() => changeMonth(1)} aria-label="Next month" className="pressable grid h-9 w-9 place-items-center rounded-md text-muted hover:bg-surface-strong hover:text-ink"><CaretRight aria-hidden size={16} /></button></div></div><div className="grid grid-cols-7 text-center text-xs font-[650] text-muted">{(weekStart === "monday" ? ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"] : ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"]).map((day) => <div key={day} className="py-2">{day}</div>)}</div><div className="grid grid-cols-7 border-l border-t border-line">{cells.map((day, index) => {
