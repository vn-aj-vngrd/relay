...
function GameList({ items }: { items: GameCollectionItem[] }) {
  return <div className="divide-y divide-line border-y border-line">{items.map((game) => <Link href={game.href} prefetch={false} key={game.id} className="pressable group flex min-h-20 items-center gap-4 py-4 hover:bg-surface sm:px-3"><time className="score w-20 shrink-0 text-sm font-bold text-primary">{game.date}</time><div className="min-w-0 flex-1"><h3 className="truncate font-[650]">{game.title}</h3><p className="mt-1 truncate text-sm text-muted">{game.time} · {game.venue}</p></div><span className="score hidden text-sm text-muted sm:block">{game.playerCount} / {game.capacity}</span><ChevronRight aria-hidden size={19} className="text-muted transition-transform group-hover:translate-x-0.5" /></Link>)}</div>;
}
