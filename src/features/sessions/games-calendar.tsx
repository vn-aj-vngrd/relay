...
export function buildCalendarDays(monthKey: string, weekStart: WeekStart): CalendarDay[] {
  const month = dateFromKey(`${monthKey}-01`);
  const monthIndex = month.getUTCMonth();
  const firstDayOffset = weekStart === "monday" ? (month.getUTCDay() + 6) % 7 : month.getUTCDay();
  const gridStart = new Date(Date.UTC(month.getUTCFullYear(), monthIndex, 1 - firstDayOffset));
...
function DayAgenda({ dateKey, games, loading }: { dateKey: string; games: CalendarGame[]; loading: boolean }) {
  const title = fullDateFormatter.format(dateFromKey(dateKey));

  return (
    <aside aria-labelledby="selected-day-heading" className="min-w-0 lg:sticky lg:top-4 lg:self-start">
      <div className="border-b border-line pb-3">
        <h3 id="selected-day-heading" className="text-base font-[680]">
          {title}
        </h3>
        <p className="mt-1 text-[13px] text-muted">
          {loading ? "Updating games…" : `${games.length} ${games.length === 1 ? "game" : "games"}`}
        </p>
      </div>
      ...
      {games.length ? (
        ...
        <div className="py-6">
          ...
          <p className="mt-1 text-sm text-muted">Choose another date to view its games.</p>
        </div>
      )}
    </aside>
  );
...
function DesktopCalendar({
  ...
  const moveSelection = (dateKey: string, amount: number) => {
    ...
    if (nextDate.slice(0, 7) !== monthKey) onMonthChange(nextDate.slice(0, 7), nextDate);
    else onSelectDate(nextDate);
  };

  return (
    <div className="hidden sm:block">
      <div role="row" className="grid grid-cols-7 border-x border-t border-line bg-surface-strong/55">
        {weekdayLabels[weekStart].map((day) => (
          <div key={day} role="columnheader" className="py-2 text-center text-xs font-[650] text-muted">
            {day}
          </div>
        ))}
      </div>
          ...
          return (
            ...
            >
              <button
                ...
                onClick={() => {
                  if (!day.inMonth) onMonthChange(day.dateKey.slice(0, 7), day.dateKey);
                  else onSelectDate(day.dateKey);
                }}
                onKeyDown={(event) => {
                  const weekday = dateFromKey(day.dateKey).getUTCDay();
                  const column = weekStart === "monday" ? (weekday + 6) % 7 : weekday;
              ...
              <div className="mt-0.5 space-y-1">
                {dayGames.slice(0, 2).map((game) => (
                  ...
                  >
                    <span aria-hidden className="h-1.5 w-1.5 shrink-0 rounded-full bg-[var(--primary)]" />
  ...
  return (
    <div className="sm:hidden">
      <div className="grid grid-cols-7">
        {weekdayLabels[weekStart].map((day) => (
          <div key={day} className="py-2 text-center text-[11px] font-[650] text-muted">
            {day.slice(0, 1)}
          </div>
        ))}
      </div>
...
export function GamesCalendar({
  ...
  const days = useMemo(() => buildCalendarDays(monthKey, weekStart), [monthKey, weekStart]);
  const games = useMemo<CalendarGame[]>(
    () => [
      ...upcoming.map((game) => ({
        ...game,
        phase: game.status === "live" ? ("live" as const) : ("upcoming" as const),
      })),
  ...
  const gamesByDate = useMemo(() => {
    ...
    for (const game of games) grouped.set(game.dateKey, [...(grouped.get(game.dateKey) ?? []), game]);
    return grouped;
  }, [games]);
  ...
  const changeMonth = (amount: number) => {
    ...
    const nextDate = nextMonth === todayKey.slice(0, 7) ? todayKey : `${nextMonth}-01`;
    ...
    <section aria-labelledby="calendar-month" aria-busy={loading}>
      <div className="mb-4 flex items-center justify-between gap-3">
        <div className="min-w-0">
          <h2 id="calendar-month" className="truncate text-xl font-[680]" aria-live="polite">
            {title}
          </h2>
          <p className="mt-1 text-[13px] text-muted">
            {loading
      ...
      {error ? (
        <div role="alert" className="mb-4 flex flex-wrap items-center justify-between gap-3 border-y border-line py-3">
          <p className="text-sm text-muted">{error} Your calendar stayed in place.</p>
          <button
            type="button"
            onClick={onRetry}
      ...
      <div className="lg:grid lg:grid-cols-[minmax(0,1fr)_18rem] lg:gap-6">
        ...
        <div className="mt-6 border-t border-line pt-5 lg:mt-0 lg:border-l lg:border-t-0 lg:pl-6 lg:pt-0">
          <DayAgenda dateKey={selectedDate} games={selectedGames} loading={loading} />
        </div>
      </div>
    </section>
  );
