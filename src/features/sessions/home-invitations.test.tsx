...
const mocks = vi.hoisted(() => ({
  rsvpAction: vi.fn(
    async (_: unknown, formData: FormData): Promise<MockActionState> => ({
      success: true,
...
afterEach(() => {
  ...
  mocks.rsvpAction.mockImplementation(
...
describe("HomeInvitations", () => {
  it("shows decision context and responds without opening the game", async () => {
    ...
    expect(
      screen.getByRole("link", { name: "Sunday Open Play" })
    ...
    expect(
      screen.getByText("Your response to Sunday Open Play was saved.")
    ).toHaveClass("sr-only");
  });
