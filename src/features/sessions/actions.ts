...
export async function createSessionAction(_: SessionActionState, formData: FormData): Promise<SessionActionState> {
  ...
  if (!parsed.success) {
    ...
    return {
      ...
      values: Object.fromEntries(["title", "accentColor", "venue", "venueAddress", "date", "capacity", "start", "end", "courts", "cost", "courtNumbers", "notes", "booked", "requiresApproval"].map((key) => [key, String(formData.get(key) ?? "")])),
      fieldErrors: {
        title: errors.title ?? [],
...
export async function updateSessionAction(_: SessionActionState, formData: FormData): Promise<SessionActionState> {
  ...
  const parsed = updateSessionSchema.safeParse({
    ...
    bookingTotalCents: typeof rawBookingTotal === "string" && rawBookingTotal ? Math.round(Number(rawBookingTotal) * 100) : undefined,
    bookingNotes: formData.get("bookingNotes") || undefined,
  });
  const savedValues = Object.fromEntries(["title", "accentColor", "venue", "venueAddress", "date", "capacity", "start", "end", "courts", "cost", "courtNumbers", "notes", "visibility", "booked", "bookingReference", "bookingTotal", "bookingNotes", "requiresApproval"].map((key) => [key, String(formData.get(key) ?? "")]));
  if (!parsed.success) {
    const errors = parsed.error.flatten().fieldErrors;
    return { error: "A few details need attention. Check the fields marked below.", values: savedValues, fieldErrors: {
      title: errors.title ?? [], venue: errors.venueName ?? [], date: errors.startsAt ?? [], start: errors.startsAt ?? [], end: errors.endsAt ?? [], capacity: errors.capacity ?? [], courts: errors.courtCount ?? [], notes: errors.notes ?? [], cost: errors.estimatedCostCents ?? [], bookingReference: errors.bookingReference ?? [], bookingTotal: errors.bookingTotalCents ?? [], bookingNotes: errors.bookingNotes ?? [],
    } };
  }
  ...
  const labels = String(formData.get("courtNumbers") ?? "").split(",").map((value) => value.trim()).filter(Boolean);
  if (parsed.data.courtCount < existing.courtCount) {
    const removedCourts = await db.select({ id: courts.id }).from(courts).where(and(eq(courts.sessionId, existing.id), gt(courts.position, parsed.data.courtCount)));
    if (removedCourts.length) {
      const matchCount = await db.$count(matches, inArray(matches.courtId, removedCourts.map((court) => court.id)));
      if (matchCount) return { error: "Court quantity can’t be reduced because matches are already assigned to a court that would be removed.", values: savedValues, fieldErrors: { courts: ["Keep the current court quantity or cancel those matches first."] } };
    }
  }
  ...
  const changedFields = [
    ...
    existing.venueName !== parsed.data.venueName || (existing.venueAddress ?? "") !== (parsed.data.venueAddress ?? "") ? "venue" : null,
    existing.startsAt.getTime() !== parsed.data.startsAt.getTime() || existing.endsAt.getTime() !== parsed.data.endsAt.getTime() ? "schedule" : null,
    existing.capacity !== parsed.data.capacity ? "player limit" : null,
    existing.courtCount !== parsed.data.courtCount || JSON.stringify(existing.courtNumbers ?? []) !== JSON.stringify(labels) ? "courts" : null,
  ...
  try {
    await db.transaction(async (tx) => {
      ...
      await tx.update(sessions).set({
        title: parsed.data.title, accentColor: parsed.data.accentColor, venueName: parsed.data.venueName, venueAddress: parsed.data.venueAddress ?? null,
        startsAt: parsed.data.startsAt, endsAt: parsed.data.endsAt, capacity: parsed.data.capacity,
        courtCount: parsed.data.courtCount, courtNumbers: labels, notes: parsed.data.notes ?? null,
        estimatedCostCents: parsed.data.estimatedCostCents ?? null, visibility: parsed.data.visibility,
        requiresApproval: parsed.data.requiresApproval,
        bookedAt: formData.get("booked") === "on" ? existing.bookedAt ?? new Date() : null,
        bookingReference: parsed.data.bookingReference ?? null, bookingTotalCents: parsed.data.bookingTotalCents ?? null,
        bookingNotes: parsed.data.bookingNotes ?? null, version: current.version + 1, updatedAt: new Date(),
      ...
      for (let index = 0; index < parsed.data.courtCount; index += 1) {
        ...
        if (currentCourt) await tx.update(courts).set({ label, version: currentCourt.version + 1, updatedAt: new Date() }).where(eq(courts.id, currentCourt.id));
        else await tx.insert(courts).values({ sessionId: existing.id, label, position });
      }
      await tx.delete(courts).where(and(eq(courts.sessionId, existing.id), gt(courts.position, parsed.data.courtCount)));

      if (changedFields.length) {
        await tx.insert(messages).values({ sessionId: existing.id, authorId: user.id, kind: "system", body: `Game details updated: ${changedFields.join(", ")}.` });
        const participants = await tx.select({ userId: sessionPlayers.userId }).from(sessionPlayers).where(eq(sessionPlayers.sessionId, existing.id));
        const recipients = [...new Set(participants.map(({ userId }) => userId).filter((id): id is string => Boolean(id) && id !== user.id))];
        if (recipients.length) await tx.insert(notifications).values(recipients.map((userId) => ({ userId, sessionId: existing.id, type: "session_details_changed", payload: { fields: changedFields } })));
      }
    });
  } catch (error) {
    if (error instanceof Error && error.message === "VERSION_CONFLICT") return { error: "Someone else updated this game. Reload the page and review their changes before saving again." };
...
const rosterManagerInput = z.object({ sessionId: z.uuid(), sessionPlayerId: z.uuid().optional(), guestName: z.string().trim().min(2, "Enter a player name.").max(60).optional() });
...
async function requireSessionManager(sessionId: string, userId: string) {
  ...
  const membership = await db.query.sessionPlayers.findFirst({ where: and(eq(sessionPlayers.sessionId, sessionId), eq(sessionPlayers.userId, userId), eq(sessionPlayers.role, "cohost")) });
  return membership ? session : null;
}

export async function addGuestPlayerAction(_: SessionActionState, formData: FormData): Promise<SessionActionState> {
  const user = await requireUser();
  const parsed = rosterManagerInput.safeParse({ sessionId: formData.get("sessionId"), guestName: formData.get("guestName") });
  if (!parsed.success || !parsed.data.guestName) return { error: parsed.success ? "Enter a player name." : parsed.error.issues[0]?.message };
  ...
  try {
    await db.transaction(async (tx) => {
      ...
      const duplicate = roster.some((player) => !player.leftAt && player.guestName?.localeCompare(parsed.data.guestName!, undefined, { sensitivity: "accent" }) === 0);
      ...
      const waitlistPosition = rsvp === "waitlisted" ? Math.max(0, ...roster.map((player) => player.waitlistPosition ?? 0)) + 1 : null;
      const [player] = await tx.insert(sessionPlayers).values({ sessionId: session.id, guestName: parsed.data.guestName, role: "player", rsvp, playState: rsvp === "going" ? "waiting" : "unavailable", waitlistPosition, respondedAt: new Date() }).returning();
      if (session.status === "live" && rsvp === "going") {
        ...
        await tx.insert(sessionQueue).values({ sessionId: session.id, sessionPlayerId: player.id, position: Math.max(0, ...queue.map((item) => item.position)) + 1, state: "waiting" });
      }
      await tx.insert(messages).values({ sessionId: session.id, authorId: user.id, kind: "system", body: `${parsed.data.guestName} was added by the host.` });
    });
  } catch (error) {
    if (error instanceof Error && error.message === "DUPLICATE_PLAYER") return { error: "A guest with this name is already on the roster." };
...
export async function approvePlayerAction(_: SessionActionState, formData: FormData): Promise<SessionActionState> {
  const user = await requireUser();
  const parsed = rosterManagerInput.safeParse({ sessionId: formData.get("sessionId"), sessionPlayerId: formData.get("sessionPlayerId") });
  ...
  try {
    ...
    const position = result === "waitlisted" ? Math.max(0, ...roster.map((item) => item.waitlistPosition ?? 0)) + 1 : null;
    await tx.update(sessionPlayers).set({ rsvp: result, waitlistPosition: position, playState: result === "going" ? "waiting" : "unavailable", respondedAt: new Date() }).where(eq(sessionPlayers.id, player.id));
    if (session.status === "live" && result === "going") {
      const queue = await tx.select().from(sessionQueue).where(eq(sessionQueue.sessionId, session.id));
      await tx.insert(sessionQueue).values({ sessionId: session.id, sessionPlayerId: player.id, position: Math.max(0, ...queue.map((item) => item.position)) + 1, state: "waiting" }).onConflictDoNothing();
    }
    await tx.insert(messages).values({ sessionId: session.id, authorId: user.id, kind: "system", body: result === "going" ? "A join request was approved." : "A join request was approved and moved to the waitlist." });
      if (player.userId) await tx.insert(notifications).values({ userId: player.userId, sessionId: session.id, type: result === "going" ? "join_approved" : "moved_to_waitlist", payload: {} });
    });
  } catch (error) {
    if (error instanceof Error && error.message === "REQUEST_GONE") return { error: "This join request was already handled." };
...
export async function removePlayerAction(_: SessionActionState, formData: FormData): Promise<SessionActionState> {
  ...
  try {
    await db.transaction(async (tx) => {
      ...
      await tx.update(sessionPlayers).set({ rsvp: "declined", playState: "unavailable", waitlistPosition: null, leftAt: new Date() }).where(eq(sessionPlayers.id, player.id));
      await tx.update(sessionQueue).set({ state: "unavailable", version: sql`${sessionQueue.version} + 1` }).where(and(eq(sessionQueue.sessionId, session.id), eq(sessionQueue.sessionPlayerId, player.id)));
      if (player.rsvp === "going") {
        const next = roster.filter((item) => item.rsvp === "waitlisted" && item.id !== player.id).sort((a, b) => (a.waitlistPosition ?? Infinity) - (b.waitlistPosition ?? Infinity))[0];
        if (next) {
          await tx.update(sessionPlayers).set({ rsvp: "going", waitlistPosition: null, playState: "waiting" }).where(eq(sessionPlayers.id, next.id));
          if (next.userId) await tx.insert(notifications).values({ userId: next.userId, sessionId: session.id, type: "moved_from_waitlist", payload: {} });
        }
      }
      await tx.insert(messages).values({ sessionId: session.id, authorId: user.id, kind: "system", body: "The host updated the player roster." });
      if (player.userId) await tx.insert(notifications).values({ userId: player.userId, sessionId: session.id, type: "removed_from_session", payload: {} });
    });
  } catch {
...
export async function markSessionBookedAction(formData: FormData) {
  ...
  if (!session.bookedAt) {
    await db.update(sessions).set({ bookedAt: new Date(), version: session.version + 1, updatedAt: new Date() }).where(and(eq(sessions.id, session.id), eq(sessions.version, session.version)));
    await db.insert(messages).values({ sessionId: session.id, authorId: user.id, kind: "system", body: "Court booking confirmed." });
    const participants = await db.select({ userId: sessionPlayers.userId }).from(sessionPlayers).where(eq(sessionPlayers.sessionId, session.id));
    const recipients = [...new Set(participants.map(({ userId }) => userId).filter((id): id is string => Boolean(id) && id !== user.id))];
    if (recipients.length) await db.insert(notifications).values(recipients.map((userId) => ({ userId, sessionId: session.id, type: "booking_confirmed", payload: {} })));
  }
  revalidatePath(`/games/${session.id}`);
  revalidatePath(`/s/${session.slug}`);
}
...
export async function toggleRosterLockAction(formData: FormData) {
  ...
  await db.update(sessions).set({ rosterLocked: !session.rosterLocked, version: session.version + 1, updatedAt: new Date() }).where(and(eq(sessions.id, session.id), eq(sessions.version, session.version)));
  revalidatePath(`/games/${session.id}/players`);
  revalidatePath(`/s/${session.slug}`);
}
...
export async function rsvpAction(_: SessionActionState, formData: FormData): Promise<SessionActionState> {
  ...
  try {
    newGuestToken = await db.transaction(async (tx): Promise<string | null> => {
      ...
      const claimingGuest = !identity && !user && parsed.data.guestName ? current.find((player) => !player.userId && !player.guestTokenHash && !player.leftAt && player.guestName?.localeCompare(parsed.data.guestName!, undefined, { sensitivity: "accent" }) === 0) : undefined;
      ...
      const nextRsvp = resolveJoinRsvp({ requested: parsed.data.choice, current: identity?.rsvp, requiresApproval: session.requiresApproval, goingCount, capacity: session.capacity });
      ...
      if (identity) {
        ...
        const claimedTokenHash = createdToken ? await crypto.subtle.digest("SHA-256", new TextEncoder().encode(createdToken)).then((value) => Buffer.from(value).toString("hex")) : identity.guestTokenHash;
        await tx.update(sessionPlayers).set({ guestTokenHash: claimedTokenHash, rsvp: nextRsvp, waitlistPosition: nextPosition, respondedAt: new Date(), playState: nextRsvp === "going" ? "waiting" : "unavailable" }).where(eq(sessionPlayers.id, identity.id));
      } else {
