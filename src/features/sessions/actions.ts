...
export async function createSessionAction(
  ...
  const result = await createSessionCommand(await requireUser(), formData);
...
export async function updateSessionAction(
  ...
  const membership = await db.query.sessionPlayers.findFirst({
    where: and(
      eq(sessionPlayers.sessionId, existing.id),
  ...
  if (parsed.data.capacity < goingCount)
    return {
      ...
      values: savedValues,
      fieldErrors: {
        capacity: ["Increase the player limit or remove players first."],
      },
    };
  ...
  if (parsed.data.courtCount < existing.courtCount) {
    const removedCourts = await db
      ...
      .where(
        and(
          eq(courts.sessionId, existing.id),
          gt(courts.position, parsed.data.courtCount)
        )
      );
    if (removedCourts.length) {
      ...
      if (matchCount)
        return {
          ...
          fieldErrors: {
            courts: [
              "Keep the current court quantity or cancel those matches first.",
            ],
          },
        };
    }
  ...
  const changedFields = [
    ...
    existing.venueName !== parsed.data.venueName ||
    (existing.venueAddress ?? "") !== (parsed.data.venueAddress ?? "")
      ? "venue"
      : null,
    ...
    existing.bookingTotalCents !==
      (parsed.data.booked ? (parsed.data.bookingTotalCents ?? null) : null) ||
    existing.bookingNotes !==
      (parsed.data.booked ? (parsed.data.bookingNotes ?? null) : null)
      ? "booking details"
      : null,
    existing.requiresApproval !== parsed.data.requiresApproval
      ? "join approval"
      : null,
  ...
  try {
    await db.transaction(async (tx) => {
      await tx.execute(
        sql`select id from ${sessions} where id = ${existing.id} for update`
      );
      ...
      for (let index = 0; index < parsed.data.courtCount; index += 1) {
        ...
        if (currentCourt)
          await tx
            ...
            .set({
              label,
              version: currentCourt.version + 1,
              updatedAt: new Date(),
            })
          ...
          await tx
            .insert(courts)
            .values({ sessionId: existing.id, label, position });
      }
      await tx
        .delete(courts)
        .where(
    ...
    if (error instanceof Error && error.message === "VERSION_CONFLICT")
      return {
        error:
          "Someone else updated this game. Reload the page and review their changes before saving again.",
      };
    console.error("Session update failed", error);
    return {
      error:
        "Game settings couldn’t be saved. Check your connection and try again.",
      values: savedValues,
    };
  }
...
const rosterManagerInput = z.object({
  ...
  playerEntry: z
    .string()
    .trim()
    .min(2, "Enter a guest name or @username.")
    .max(60)
    .optional(),
  ...
  if (!parsed.success || !parsed.data.playerEntry)
    return {
      error: parsed.success
        ? "Enter a guest name or @username."
...
export async function approvePlayerAction(
  _: SessionActionState,
  formData: FormData
): Promise<SessionActionState> {
  const user = await requireUser();
  ...
  if (!parsed.success || !parsed.data.sessionPlayerId)
    return { error: "This join request could not be found." };
  return manageRoster({
    type: "approve",
    actorUserId: user.id,
    sessionId: parsed.data.sessionId,
...
export async function markSessionBookedAction(formData: FormData) {
  ...
  const session = await requireSessionManager(sessionId, user.id);
  if (!session)
    throw new Error("Only a host or co-host can confirm the booking");
  if (!session.bookedAt) {
    await db
      .update(sessions)
      .set({
...
async function attendanceMutationContext(
  sessionId: string,
  ...
  if (target?.rsvp !== "going")
    return { error: "Only going players can update availability." } as const;

  const user = await getCurrentUser();
  const manager = user
    ? await requireSessionManager(session.id, user.id)
    : null;
  let isSelf = Boolean(user && target.userId === user.id);
  if (!user && target.guestTokenHash) {
    const guestToken = (await cookies()).get(
      `relay_guest_${session.id}`
    )?.value;
    if (guestToken) {
  ...
  if (!manager && !isSelf)
    return { error: "You can only update your own availability." } as const;
  if (!manager) {
    ...
    if (!limit.allowed)
      return {
        error:
          "Availability is changing too quickly. Wait a moment and try again.",
      } as const;
  }
...
export async function setAttendanceAction(
  ...
  const context = await attendanceMutationContext(
    ...
    await db.transaction(async (tx) => {
      ...
      const currentQueue = await tx.query.sessionQueue.findFirst({
        where: and(
          ...
          eq(sessionQueue.sessionPlayerId, target.id)
        ),
      });
      if (!parsed.data.present && currentQueue?.state === "playing")
        ...
        if (parsed.data.present && !currentQueue) {
          ...
          await tx
            ...
            .set({
              ...
              enteredAt: parsed.data.present
                ? new Date()
                : currentQueue.enteredAt,
...
export async function setPlayAvailabilityAction(
  ...
  if (!parsed.success)
    return { error: "This player’s availability could not be updated." };
  const context = await attendanceMutationContext(
    parsed.data.sessionId,
    parsed.data.sessionPlayerId
  );
  ...
  const { session, target } = context;
  if (session.status !== "live")
    return { error: "Play availability opens after Play starts." };
  ...
  try {
    await db.transaction(async (tx) => {
      ...
      if (currentSession?.status !== "live")
        throw new Error("AVAILABILITY_CLOSED");
      if (currentTarget?.rsvp !== "going") throw new Error("PLAYER_NOT_FOUND");
      const queue = await tx
      ...
      await tx
        ...
        .set({
          checkedInAt:
            parsed.data.intent === "ready"
              ? (currentTarget.checkedInAt ?? new Date())
              : currentTarget.checkedInAt,
    ...
    console.error(
      "Play availability update failed",
...
export async function setAllAttendanceAction(
  ...
  if (!session)
    return { error: "Only a host or co-host can update everyone’s arrival." };
  if (!["draft", "published", "live"].includes(session.status))
  ...
  try {
    await db.transaction(async (tx) => {
      ...
      if (
        ...
        queue.some(
          (item) =>
            playerIds.includes(item.sessionPlayerId) && item.state === "playing"
        )
      )
        throw new Error("PLAYER_ACTIVE");
      ...
      const updateIds = parsed.data.present
        ? playerIds.filter((id) => !playingIds.has(id))
        : playerIds;
      if (updateIds.length)
        await tx
          .update(sessionPlayers)
...
export async function rsvpAction(
  ...
  if (!limit.allowed)
    return {
      error:
        "Responses are being updated too quickly. Wait a moment and try again.",
    };
  if (!user && !parsed.data.guestName && !guestToken)
    return { error: "Add your name before responding." };
  const accountProfile = user
    ...
    : null;
  const selectedExperience = user
    ? (accountProfile?.skillLevel ?? null)
    : (parsed.data.skillLevel ?? null);
  ...
  try {
    const result = await db.transaction(
      async (
        tx
      ): Promise<{ guestToken: string | null; reachedFourth: boolean }> => {
        await tx.execute(
          sql`select id from ${sessions} where id = ${session.id} for update`
        );
        const currentSession = await tx.query.sessions.findFirst({
        ...
        const current = await tx
          ...
          .from(sessionPlayers)
          .where(eq(sessionPlayers.sessionId, session.id));
        let identity = findRosterIdentity(current, {
          userId: user?.id,
          ...
          !identity && !user && parsed.data.guestName
            ? current.find(
                (player) =>
                  ...
                  !player.leftAt &&
                  player.guestName?.localeCompare(
        ...
        const goingBefore = current.filter(
        ...
        if (identity) {
          ...
          await tx
            ...
            .set({
              ...
              skillLevel: user
                ? selectedExperience
                : (selectedExperience ?? identity.skillLevel),
              ...transition.target,
        ...
        if (currentSession.status === "live" && actorPlayerId) {
          const queueEntry = await tx.query.sessionQueue.findFirst({
            where: and(
              eq(sessionQueue.sessionId, session.id),
              eq(sessionQueue.sessionPlayerId, actorPlayerId)
            ),
          });
          if (nextRsvp === "going" && !queueEntry) {
            ...
            await tx.insert(sessionQueue).values({
              ...
              sessionPlayerId: actorPlayerId,
              position:
                Math.max(0, ...currentQueue.map((item) => item.position)) + 1,
              state: "waiting",
            });
        ...
        const rsvpByPlayer = new Map(
          transition.updates.map((update) => [update.id, update.rsvp])
        );
        if (actorPlayerId) rsvpByPlayer.set(actorPlayerId, nextRsvp);
        const goingAfter =
          current.filter(
            (player) => (rsvpByPlayer.get(player.id) ?? player.rsvp) === "going"
          ).length + (!identity && nextRsvp === "going" ? 1 : 0);
        return {
          guestToken: createdToken,
          reachedFourth: goingBefore < 4 && goingAfter >= 4,
        };
      }
    );
    ...
    return {
      error:
        "Your response couldn’t be saved. Check your connection and try again.",
    };
  }
  if (newGuestToken)
    ...
    ...(parsed.data.inviteSource
      ? [
          trackProductEvent({
            ...
            source: user ? ("authenticated" as const) : ("guest" as const),
            metadata: {
              inviteSource: parsed.data.inviteSource,
