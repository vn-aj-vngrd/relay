...
describe("AddPlayerForm", () => {
  it("switches between adding a guest and finding a Relay username", () => {
    ...
    const entry = screen.getByRole("combobox", {
      name: "Guest name or Relay username",
    });
    expect(screen.getByRole("button", { name: "Add" })).toBeVisible();
    expect(
      screen.getByRole("button", { name: "Guest playing experience" })
    ).toBeEnabled();
    ...
    expect(
      screen.queryByRole("button", { name: "Guest playing experience" })
  ...
  it("suggests Relay players and requires a resolved selection before inviting", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      "/api/search?q=mi&type=players&cursor=0",
      expect.any(Object)
    );
    expect(screen.getByRole("button", { name: "Invite" })).toBeDisabled();
  ...
  it("clears a guest name after the guest is added", async () => {
    vi.mocked(addPlayerAction).mockResolvedValueOnce({
      success: true,
      playerOutcome: "added",
    });
    render(<AddPlayerForm sessionId="59c6fa3f-3f6f-45f2-bbea-b85bc90aa3a7" />);
...
describe("RemovePlayerButton", () => {
  it("explains an unlabeled icon and uses a Relay confirmation dialog", () => {
    ...
    expect(
      screen.getByText(/lose their spot and payment assignment/i)
    ).toBeVisible();
