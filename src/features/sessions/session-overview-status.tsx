...
function responseDetail(rsvp?: string) {
  if (rsvp === "waitlisted") return "Relay will keep your place in line if a spot opens.";
  if (rsvp === "maybe") return "Update your response when you know whether you can play.";
  return "You’re confirmed for this game.";
}

function paymentAction(payment: SessionOverview["payment"]) {
  if (payment.view !== "player" || payment.status === "confirmed" || payment.status === "excluded") return null;
  if (payment.reviewRequested) return `Upload new proof · ${peso(payment.amountCents)}`;
...
export function SessionOverviewStatus({
  ...
  const shellClass = embedded ? "bg-surface py-5" : "rounded-xl border border-line bg-surface p-4 sm:p-5";
  ...
  if (completed)
    return (
      <section className={`${shellClass} ${className}`}>
        ...
        <p className="mt-2 text-sm leading-6 text-muted">Scores, standings, payments, chat, and photos stay here.</p>
        <div className="mt-5 space-y-2">
          <ButtonLink href={`/games/${sessionId}/play`} className="w-full">
            View recap
          </ButtonLink>
          {playerPaymentAction ? (
            <ButtonLink href={`/games/${sessionId}/payments`} variant="secondary" className="w-full">
              {playerPaymentAction}
            </ButtonLink>
          ) : null}
          {canReplay ? (
  ...
  return (
    <section className={`${shellClass} ${className}`}>
      ...
      <p className="mt-2 text-sm leading-6 text-muted">
        {isHost ? "Finish the game setup, then open Play when the group reaches the court." : responseDetail(rsvp)}
      </p>
