...
export function planRosterTransition(input: {
  ...
  if (input.capacity < 2)
    throw new Error("Session capacity must be at least 2");

  const current = input.intent.playerId
    ? input.roster.find((player) => player.id === input.intent.playerId)
    : undefined;
  const otherPlayers = input.roster.filter(
    (player) => player.id !== input.intent.playerId
  );
  const goingCount = otherPlayers.filter(
    (player) => player.rsvp === "going"
  ).length;
  const nextRsvp = requestedRsvp(
    input.intent,
    current,
    goingCount,
    input.capacity
  );
  const lastWaitlistPosition = Math.max(
    0,
    ...otherPlayers.map((player) => player.waitlistPosition ?? 0)
  );
  ...
  const planned = [
    ...otherPlayers.map((player) => ({ ...player })),
    {
      ...
      waitlistPosition:
        nextRsvp === "waitlisted"
          ? (current?.waitlistPosition ?? lastWaitlistPosition + 1)
          : null,
    },
  ];
  ...
  if (openedPlace) {
    const next = planned
      ...
      .toSorted(
        (left, right) =>
          (left.waitlistPosition ?? Number.POSITIVE_INFINITY) -
          (right.waitlistPosition ?? Number.POSITIVE_INFINITY)
      )[0];
    if (next) {
      next.rsvp = "going";
  ...
  const updates = planned.flatMap((player) => {
    ...
    if (
      !previous ||
      (previous.rsvp === player.rsvp &&
        previous.waitlistPosition === player.waitlistPosition)
    )
      return [];
