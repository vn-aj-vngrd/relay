...
function SessionFormActions() {
  ...
  return <div className="flex flex-col-reverse gap-3 border-t border-line pt-6 sm:flex-row sm:items-center sm:justify-between"><p className="text-sm leading-5 text-muted">Publishing creates the link to share. You can change the plan later.</p><Button type="submit" name="intent" value="publish" className="w-full sm:w-auto sm:min-w-40" disabled={pending}>{pending ? <><ButtonSpinner />Publishing…</> : "Publish game"}</Button></div>;
}
...
export function QuantityInput({ id, label, hint, min, max, initialValue, error }: { id: string; label: string; hint: string; min: number; max: number; initialValue?: number; error?: string }) {
  ...
  return <div className="min-w-0">
    ...
    <div className={`mt-1.5 flex h-11 w-full items-stretch overflow-hidden rounded-lg border bg-surface ${error ? "border-danger ring-2 ring-danger/10" : "border-line focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/15"}`}>
      <button type="button" onClick={() => changeBy(-1)} disabled={!Number.isFinite(numericValue) || numericValue <= min} aria-label={`Decrease ${label.toLowerCase()}`} className="pressable grid w-12 shrink-0 place-items-center border-r border-line text-muted hover:bg-surface-strong hover:text-ink disabled:opacity-35"><Minus aria-hidden size={17} /></button>
      <input id={id} name={id} type="number" inputMode="numeric" min={min} max={max} step="1" value={value} onChange={(event) => setValue(event.target.value)} required aria-invalid={Boolean(error)} aria-describedby={describedBy} className="score min-w-0 flex-1 appearance-none bg-transparent px-2 text-center text-base font-semibold text-ink outline-none [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none" />
      <button type="button" onClick={() => changeBy(1)} disabled={numericValue >= max} aria-label={`Increase ${label.toLowerCase()}`} className="pressable grid w-12 shrink-0 place-items-center border-l border-line text-muted hover:bg-surface-strong hover:text-ink disabled:opacity-35"><Plus aria-hidden size={17} /></button>
    </div>
...
export type CreateSessionDefaults = { date?: string; title?: string; venue?: string; venueAddress?: string; capacity?: number; courts?: number; start?: string; end?: string; cost?: number; accentColor?: string; groupId?: string; groupName?: string; sourceSessionId?: string; inviteeCount?: number };
...
export function CreateSessionForm({ defaults }: { defaults: CreateSessionDefaults }) {
  ...
  const value = (field: string, initial?: string | number) => state.values
    ? state.values[field] ?? ""
    : initial == null ? "" : String(initial);
  const advancedOpen = more || Boolean(
    costError || notesError || state.values?.cost || state.values?.courtNumbers || state.values?.notes ||
    state.values?.booked || state.values?.requiresApproval || defaults.cost != null ||
  ...
  return <form className="space-y-8" action={action} autoComplete="off" noValidate>
    ...
    {defaults.sourceSessionId ? <input type="hidden" name="sourceSessionId" value={defaults.sourceSessionId} /> : null}
    ...
    {defaults.groupName ? <section className="border-y border-line py-4"><p className="text-sm font-semibold">For {defaults.groupName}</p><p className="mt-1 text-sm text-muted">{defaults.inviteeCount ? `${defaults.inviteeCount} group ${defaults.inviteeCount === 1 ? "member" : "members"} will be invited when you publish.` : "No other members yet. Add players from the group page or share the game link."}</p></section> : defaults.sourceSessionId && defaults.inviteeCount ? <section className="border-y border-line py-4"><p className="text-sm font-semibold">Familiar crew ready</p><p className="mt-1 text-sm text-muted">{defaults.inviteeCount} signed-in players from the last game will be invited again. Their previous RSVP is not copied.</p></section> : null}

    <section className="space-y-6" aria-labelledby="game-basics-heading">
      <div><h2 id="game-basics-heading" className="text-lg font-[680]">The plan</h2><p className="mt-1 text-sm text-muted">Give friends enough context to recognize the game and find the court.</p></div>
      <div>
        <label className={labelClass} htmlFor="title">Game name</label>
        <input className={fieldClass(titleError)} id="title" name="title" required minLength={2} maxLength={80} placeholder="Saturday night pickle" defaultValue={value("title", defaults.title)} aria-invalid={Boolean(titleError)} aria-describedby={titleError ? "title-error" : "title-hint"} />
        <p id="title-hint" className="mt-1.5 text-sm text-muted">Use a name your group will recognize.</p>
    ...
    <section className="space-y-6 border-t border-line pt-8" aria-labelledby="setup-heading">
      <div><h2 id="setup-heading" className="text-lg font-[680]">Game setup</h2><p className="mt-1 text-sm text-muted">Relay uses these numbers for waitlisting and court organization.</p></div>
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 sm:gap-4">
        <QuantityInput id="capacity" label="Player limit" hint="2–40 players. Extra players join the waitlist." min={2} max={40} initialValue={value("capacity", defaults.capacity) ? Number(value("capacity", defaults.capacity)) : undefined} error={capacityError} />
        <QuantityInput id="courts" label="Court quantity" hint="1–20 courts. You can label them later." min={1} max={20} initialValue={value("courts", defaults.courts) ? Number(value("courts", defaults.courts)) : undefined} error={courtsError} />
      </div>
    </section>
    ...
    <section className="border-t border-line py-2">
      ...
      {advancedOpen ? <div className="space-y-6 pb-5 pt-4">
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 sm:gap-4">
          ...
          <div className="min-w-0"><label className={labelClass} htmlFor="court-numbers">Court labels</label><input className={fieldClass()} id="court-numbers" name="courtNumbers" placeholder="2, 3, Center" defaultValue={value("courtNumbers")} /><p className="mt-1.5 text-sm text-muted">Optional names shown in Play.</p></div>
        </div>
