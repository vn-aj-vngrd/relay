...
async function hashToken(token: string) {
  return crypto.subtle.digest("SHA-256", new TextEncoder().encode(token)).then((value) => Buffer.from(value).toString("hex"));
}
...
export async function getSessionViewer(sessionId: string, slug: string) {
  ...
  const player = await db.query.sessionPlayers.findFirst({ where: and(eq(sessionPlayers.sessionId, sessionId), eq(sessionPlayers.guestTokenHash, tokenHash)) });
