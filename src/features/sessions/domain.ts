...
export function resolveJoinRsvp(input: {
  requested: "going" | "maybe" | "declined";
  current?: Rsvp;
  requiresApproval: boolean;
  goingCount: number;
  capacity: number;
}): "going" | "maybe" | "pending" | "waitlisted" | "declined" {
...
export function applyRsvp(
  roster: RosterPlayer[],
  playerId: string,
  requested: Exclude<Rsvp, "invited" | "pending" | "waitlisted">,
  capacity: number,
): RosterPlayer[] {
  ...
  return [
    ...withoutPlayer,
    {
      id: playerId,
      rsvp,
      ...(rsvp === "waitlisted" ? { waitlistPosition: existing?.waitlistPosition ?? maxWaitlist + 1 } : {}),
    },
  ];
}
