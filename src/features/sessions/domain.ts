...
export const updateSessionSchema = sessionDetailsSchema.extend({
  sessionId: z.uuid(),
  ...
  bookingTotalCents: z.coerce.number().int("Enter a valid booking total.").nonnegative("Booking total can’t be negative.").optional(),
  bookingNotes: z.string().trim().max(600, "Keep booking notes under 600 characters.").optional(),
}).superRefine(validateTimes);

export type Rsvp = "invited" | "pending" | "going" | "maybe" | "waitlisted" | "declined";
...
export function resolveJoinRsvp(input: { requested: "going" | "maybe" | "declined"; current?: Rsvp; requiresApproval: boolean; goingCount: number; capacity: number }): "going" | "maybe" | "pending" | "waitlisted" | "declined" {
  if (input.requested !== "going") return input.requested;
  if (input.requiresApproval && input.current !== "going" && input.current !== "waitlisted") return "pending";
  return input.goingCount >= input.capacity ? "waitlisted" : "going";
}

export function applyRsvp(roster: RosterPlayer[], playerId: string, requested: Exclude<Rsvp, "invited" | "pending" | "waitlisted">, capacity: number): RosterPlayer[] {
