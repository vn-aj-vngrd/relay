...
Live Courts includes the shared Up next section beneath scoreboards in Quick
Play, authenticated Play and shared Play. Busy Paddle Stack courts show the
next teams when four waiting players can rotate in, or the eligible players who
should prepare when results determine the remaining places. The court remains
unassigned until one opens. Synchronized formats finalize teams after every
court finishes.
Ready lineups show exact teams and courts with the organizer's Start action.
Saved-game starts compare the displayed lineup with a fresh plan under the
session lock and request review on change. Quick Play remains device-local.
