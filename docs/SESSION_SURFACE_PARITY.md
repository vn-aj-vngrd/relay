...
- Use the 896px session canvas and the spacing rules in `DESIGN.md`.
...
- Subscribe only to shared changing state: active matches, scores, queue, and chat.
- Reconnect language and concurrency errors must match across access paths.
- Optimistic score changes remain host-only and reconcile against the server version.
