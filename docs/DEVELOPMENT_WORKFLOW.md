...
4. Fill in the PR template with final behavior, concrete test evidence, and material risks. Include an **Actions and decisions** table in every PR, immediately after Summary, using the columns `Area / finding`, `Decision and reason`, `Change / action`, and `Result / evidence`. Give each meaningful finding or change one concise row, including investigated items kept unchanged and blocked or deferred items. Distinguish implemented, verified, failed, and unverified results; name the remaining action for blockers. Update the table to match the final diff and latest evidence before merge. Complete when a reader can understand what changed, why, what was deliberately left alone, and what actually passed without reading the conversation. Keep screenshots for visual changes when captured; label unrun checks and synthetic fixtures accurately. Use `None` for sections without applicable details.
