...
| J13 | Host/player previews and downloads Story | Lifecycle facts match the game; Court Pop opens by default; visible theme selection preserves photo/crop/message; subject, art and practical copy do not overlap; export uses the same geometry; failures offer recovery. | `recap-share-card.test.tsx`, `story-theme.test.ts`, invitation/recap layout suites and `e2e/story-creative.spec.ts`. New Court Pop coverage is authored but unrun. Earlier PNG/layout evidence does not validate this redesign; actual mobile OS download/share remains manual. |
