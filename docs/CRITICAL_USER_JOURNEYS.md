...
| J15 | Player reviews personal game insights | Own Profile links to private insights on every viewport, with an additional Home link on desktop; completed hosted games and recorded play totals agree with saved match scores; unscored RSVPs and local Quick Play are excluded; recent games link to Play. | `src/features/players/insights.test.ts`, Home link test, route access test and breadcrumb test cover query scope, privacy and navigation. Authenticated browser verification remains opt-in. |
| J16 | Player navigates secondary pages on a phone | Search, Profile, Notifications, Settings, billing, feedback, Courts and Help show a compact back-and-title bar with a working parent route; Home, Games and Groups keep their primary navigation; page actions and content remain available without a duplicate mobile title. | `secondary-mobile-header.test.ts` covers route mapping. Signed-in browser checks at 390px covered Search, Notifications, Profile, Settings, Courts and Help; other secondary pages and dark mode remain unverified. |
...
| --- | --- | --- |
| A01 — Ask Agent about games, rosters, groups or Help Center | Authenticated active account; past/current/upcoming/all/draft games and distinct My Games/Invitations; group discovery, details and members; authorized Play/Recap, payment, chat and Story reads; partial lists disclose limits; only authorized data; streaming answer with source links; bounded activity labels and elapsed time expand while working and collapse on completion; stopped/failed activity persists; response errors and Retry stay in the affected Agent turn; no unapproved writes; cancellation and safe recovery | Agent request/read/search/game-sections/tool/route/help unit regressions; synthetic responsive chat scenario in `e2e/agent-chat.spec.ts`. Local 2026-09-25: all 2,661 unit tests and six Agent browser cases passed, including discovery and reload at 390px/1440px in mobile/desktop Chromium. Lint, types and production build passed. Live provider and cross-account database exercise remain unverified. |
...
| A04 — Continue an Agent conversation | Sidebar and mobile header show working, unread completion and failure indicators; recent chats and history show reply state; returning to Agent clears unread status; in-app navigation preserves pending chat creation, streaming replies, activity, selected conversation and draft; one reply runs per account and new chats wait until it finishes; sign-out/account changes stop and clear the active session; refresh restores the selected saved chat; owner-only rename, archive, restore and delete; Chats and Archived stay separate; account changes isolate state; Markdown renders without HTML or unsafe links; message time and Copy appear on hover or keyboard focus, remain visible on touch, and never shift layout | Agent history/auth/stream, session, answer and reply-actions unit regressions; synthetic Agent navigation fixture covers message actions for both authors. Archive and account-concurrency behavior added; validation deferred to pre-commit. |
