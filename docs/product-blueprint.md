...
Creation and Game settings → Payments share repayment setup fields. A creation collection is persisted with its payment account atomically with the game; the host paid upfront and is excluded. Collections hold up to 20 named expense items and an explicit contribution method. Fixed amount per player supplies a stated price before the roster exists; split expenses remains pending until a share exists, never Free. A fixed amount is independent of actual expenses and can yield a shortfall or surplus; settings shows expected contributions separately. A game has one contribution method across its collections, with at most 20 collections. Fixed collection rates are summed for discovery; current split prices use the highest aggregate assigned share. Signed-out drafts retain only serializable fields; QR and receipt images are added in settings and never saved locally.
