...
3. Completes **Players and access**: capacity, court count, visibility, and approval behavior. Payment choice defaults to Decide later (unset), or the host chooses Free or Collect payment. Collect payment saves intent only; price, expenses, contribution method, and payment instructions are configured afterward in Game settings → Payments. Overview offers the original host a compact Set player payment or Set up payment link. Public games without a stated player price can be shared but are not discoverable.
...
Creation and Game settings → Payments share payment choices, but new creation drafts collect only intent. Settings owns the full repayment setup; a collection is not created until that setup is saved. Existing browser drafts containing setup details retain editable fields and the legacy atomic game/account/collection publication path. Collection intent is stored independently from player price, creates no obligation, and is not copied by Play Again. For configured collections, the host paid upfront and is excluded. Collections hold up to 20 named expense items and an explicit contribution method. Fixed amount per player supplies a stated price before the roster exists; split expenses remains pending until a share exists, never Free. A fixed amount is independent of actual expenses and can yield a shortfall or surplus; settings shows expected contributions separately. A game has one contribution method across its collections, with at most 20 collections. Fixed collection rates are summed for discovery; current split prices use the highest aggregate assigned share. Signed-out drafts retain only serializable fields; QR and receipt images are added in settings and never saved locally.

Settings is the ongoing configuration home, including Free corrections and collection/share changes. Payments tracks repayments, accepts proof, and lets organizers review and confirm, with direct setup/edit links. The original host may switch active collections to Free after confirmation against the current payment snapshot. This archives every active collection, cancels unpaid obligations and pending proposals, retains proof and amounts in read-only history, and notifies affected players once. Submitted or paid records require manual host follow-up; no refund is implied or issued. Collection history still blocks Decide later. Switching Free back to Collect payment requires confirmation of a fresh setup and consent from existing players; old obligations are never restored. New collection cycles are unavailable after completion. Pricing, reconciliation, and mutation eligibility exclude archived collections; both Payments surfaces separate current obligations from retained history. No records are erased. Any existing player share, including unpaid or excluded shares, prevents collection-total edits; payment instructions remain correctable. Contribution mode and fixed rates lock after any game share exists, and fixed games cannot add further required collections after that point. Manual discounts/waivers carry a reason and persist across roster reconciliation without redistributing the difference. Increases are stored as proposals without changing the existing amount; the affected signed-in or token-bound guest player accepts or declines the exact proposal, with append-only adjustment history. Declining preserves the prior amount as a manual override. A host/co-host may propose a missing share without recalculating other players. Previously Free games request agreement before charging existing players. Fixed-price late joiners receive the stored rate even if another player already sent proof. Reviewed split collections remain frozen. Existing shares are migrated as legacy/unknown, never guessed automatic: legacy split recalculation pauses until organizers explicitly manage the amounts. Payment follow-up stays available after completion and closes on cancellation.

Host Overview renders the same discovery predicate as Open games, search, and signed-in nonmember access: public, published/live, scheduled end strictly in the future, and non-null price. Name, schedule, court name, and capacity are already required by the game domain; there are no additional booking/receipt/QR/Play readiness gates. Private/link access is separate from listing eligibility. Shared price disclosure differentiates unset, Free, pending split, fixed rate, and current variable share across both route families and game collections. Migration `0048_collection_contributions` is additive and must be applied separately before deploying the new payment queries.
