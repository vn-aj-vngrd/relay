...
Use the same above-input popover surface as the + actions menu. Guided questions is the default. Offer Answer in chat for conversational collection of one missing answer at a time and Answer with choices to reopen the panel. Persist interactionMode with the draft; both modes end in the same explicit approval. Keep chat and the composer interactive; present focused questions with step tabs, answer choices and custom inputs. Reuse named controls from Create Game and Quick Play. Game/draft: court and schedule, players/settings, review. Replay/group game/crew: select an eligible source first. Group: name/description, review. Quick Play: names, courts/format, review. Chat preparation opens a form with known fields immediately instead of asking numbered questions. Only the ambiguous hosted-game versus local-Quick-Play intent needs conversational clarification.

Setup is an owner-scoped proposal with a server-owned `preview.collecting` marker; its public status is collecting and it cannot execute. Next and Save and close persist setup, not product resources. Outside clicks leave the popover and answers intact. All viewports use the composer-width popover above the input with a viewport-bounded scrolling body; never replace it with a modal or bottom sheet. Save current answers before sending a conversational correction; creationStatus supplies these answers for the model to merge. Review validates through the existing domain rules and creates a new immutable proposal ID. Editing a review cancels the old ID before accepting more changes. A stable client request ID recovers the replacement after a lost transition response; failed saves also offer a local dismissal that retains answers while the page remains mounted. Final approval uses the existing ID-only confirmation endpoint. Keep this boundary when adding steps; client fields and typed yes never authorize execution.
