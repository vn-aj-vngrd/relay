...
Management controls sit beside the information they change and render only for hosts or co-hosts. Game settings is a focused route with no game workspace tabs, a Back to game action, and breadcrumbs ending in Game settings. Its compact tabs are Plan, Invite, Booking, and Organizers. Organizer information and authority changes live only in Organizers, separate from roster management: before Play, the original Host may add any Relay member by username as a non-playing Co-host, promote an existing account player to Co-host, or remove Co-host access, while Co-hosts can view the organizer team. Players remains participation-focused and marks Host or Co-host beside any organizer who appears in a roster state. Before play, the host Overview’s Set up Play action opens arrival and rotation setup directly; once live it opens Play. Sharing is the persistent session action, so inviting someone never requires returning to another tab. Player screens remain readable and action-light; they are not disabled host dashboards.
