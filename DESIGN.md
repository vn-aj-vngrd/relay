...
Customization is broad but bounded: four structural layouts, Relay palettes, persisted session photos, a local device photo, vertical crop, text contrast, a short custom headline, and one personal line. Five Story themes keep Minimal as the unchanged default: Scrapbook uses layered photo paper and taped-in paddle art; Coquette uses a statement ribbon, blush paddles, pearls, and lace edging; Court Pop uses bright court geometry and a perforated sport ball; Retro Rally uses vintage poster stripes and a paddle stamp. A Story-only Pink background uses dark text without changing the game or app accent. Expressive artwork occupies a dedicated masthead band and outer framing, separate from facts and controls; the surrounding editor stays minimal. The illustrated sport ball is distinct from Relay’s unchanged brand mark. Theme geometry and numeric font choices are shared by the HTML preview and PNG renderer. Expressive cards reserve header, illustration, and factual regions; long preview content scales within its region without dropping names, and expanded preview remains the reading surface. Framed keepsake, scalloped invitation, court poster, and striped ticket outlines connect each illustration to its factual panel. Minimal keeps its original typography and geometry. Local background photos stay on-device unless separately added to the session. Preview and exported 1080 × 1920 PNG must carry the same focus, layout, crop, contrast, and words. Native file sharing is used only when supported; explicit PNG download remains available. Story never becomes a competitive report or an unconstrained drag-and-drop editor that can fabricate results or break export parity.
