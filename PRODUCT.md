...
1. **Plan and invite:** the host moves through a short Plan → Players and access → optional Details → read-only Review flow, sets the schedule, court, roster, and visibility, optionally records booking details, then publishes one shared game link. Payment choice defaults to Decide later (unset), with Free or Collect payment available before publication. Collect payment records intent only; Overview links the original host to Game settings → Payments to set expenses, price, and instructions afterward. Public games remain shareable but undiscoverable until a player price is stated. Configured collections itemize upfront expenses. Hosts choose variable expense splitting or a fixed contribution per player; fixed rates are stated before anyone joins. Individual discounts and waivers persist, while increases require the affected player’s agreement. Game settings → Payments owns ongoing configuration, including confirmed switches between Free and Collect payment. Closing collections retains records and proof without implying refunds; collecting again starts a new cycle with consent from existing players. Payments owns current tracking, proof review, and read-only history.
...
- Anyone may browse public, unended sessions that disclose Free or a current split share or an explicit fixed per-player contribution. Host Overview explains whether the game is listed and exactly what prevents listing; optional setup never becomes a 100% completion gate. Signed-out visitors open the shared game and authenticate when they want account identity or history. Link-only sessions remain accessible only by shared link and private sessions only to hosts and invited participants.
...
8. **Public means informed.** A discoverable game states its court, roster limit, admission behavior, and player price before a new player joins.
