...
1. **Plan and invite:** the host moves through a short Plan → Players and access → optional Details → read-only Review flow, sets the schedule, court, roster, and visibility, optionally records booking details, then publishes one shared game link. Optional Player payment defaults to Decide later (unset), with Free or Collect payment available before publication. Collections itemize upfront expenses. Hosts choose variable expense splitting or a fixed contribution per player; fixed rates are stated before anyone joins. Individual discounts and waivers persist, while increases require the affected player’s agreement. Game settings → Payments owns ongoing configuration; Payments owns tracking and proof review.
