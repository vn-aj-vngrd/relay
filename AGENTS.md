...
- **PR workflow:** Start new work on a `van/` branch from freshly fetched `origin/master`. Read `docs/DEVELOPMENT_WORKFLOW.md` before branching, committing, opening a PR, or merging. Changes reach `master` through a PR with passing checks; never push commits directly to `master`. Creating a PR does not authorize merging it.
